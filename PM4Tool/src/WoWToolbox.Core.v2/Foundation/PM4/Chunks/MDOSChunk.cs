...
namespace WoWToolbox.Core.v2.Foundation.PM4.Chunks
{
    ...
    public class MdosEntry
    {
        ...
        // --- Extra fields referenced by exporter/validators ---
        ...
        public const int Size = 24; // Adjusted placeholder size - Reverted to PM4 docs

        public void Load(BinaryReader br)
        {
            // Basic reading for mandatory first two fields
            m_destructible_building_index = br.ReadUInt32();
            destruction_state = br.ReadUInt32();

            // Attempt to read optional fields safely if stream has enough data
            if (br.BaseStream.Position + 4 <= br.BaseStream.Length) NameId = br.ReadUInt32();
            if (br.BaseStream.Position + 2 <= br.BaseStream.Length) DoodadSet = br.ReadUInt16();
            if (br.BaseStream.Position + 4 <= br.BaseStream.Length) Flags = br.ReadUInt32();
            if (br.BaseStream.Position + 4 <= br.BaseStream.Length) Scale = br.ReadSingle();
            /* Reverted
            for (int j = 0; j < 32; j++)
            {
