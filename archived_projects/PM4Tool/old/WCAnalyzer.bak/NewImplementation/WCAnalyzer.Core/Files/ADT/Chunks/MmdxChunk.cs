...
{
    /// <summary>
    /// MMDX chunk - Contains M2 model filenames (doodads)
    /// </summary>
    public class MmdxChunk : ADTChunk
    {
        /// <summary>
        /// The MMDX chunk signature
        /// </summary>
        public const string SIGNATURE = "MMDX";

        /// <summary>
        /// Gets the list of model filenames
        /// </summary>
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="MmdxChunk"/> class
        /// </summary>
        /// <param name="data">The chunk data</param>
        /// <param name="logger">Optional logger</param>
        public MmdxChunk(byte[] data, ILogger logger = null) : base(SIGNATURE, data, logger)
        {
            Parse();
        }

        /// <summary>
        /// Parses the MMDX chunk data
        /// </summary>
        protected override void Parse()
        {
            try
            {
                ...
                {
                    Logger?.LogWarning("MMDX chunk has no data");
                    return;
                }
                ...
                Logger?.LogDebug($"MMDX: Read {Filenames.Count} model filenames");
            }
            catch (Exception ex)
            {
                Logger?.LogError($"Error parsing MMDX chunk: {ex.Message}");
                throw;
            }
        }
        ...
        public override void Write(BinaryWriter writer)
        {
            if (writer == null)
            {
                Logger?.LogError("Cannot write MMDX chunk: BinaryWriter is null");
            ...
            try
            {
                ...
                Logger?.LogDebug($"MMDX: Wrote {Filenames.Count} model filenames");
            }
            catch (Exception ex)
            {
                Logger?.LogError($"Error writing MMDX chunk: {ex.Message}");
                throw;
            }
        }
