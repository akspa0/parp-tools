...
{
    /// <summary>
    /// MSPV chunk - Contains position vertices for the PD4 format
    /// </summary>
    public class MspvChunk : PD4Chunk
    {
        /// <summary>
        /// Signature for this chunk ("MSPV")
        /// </summary>
            ...
            : base(SIGNATURE, data, logger)
        {
            ...
            if (_data.Length % VERTEX_SIZE != 0)
            {
                Logger?.LogWarning($"MSPV chunk has irregular size: {_data.Length} bytes. Not a multiple of {VERTEX_SIZE} bytes per vertex.");
            }
        }
        ...
        {
            return _data.Length / VERTEX_SIZE;
        }
        ...
        {
            byte[] newData = new byte[_data.Length + VERTEX_SIZE];
