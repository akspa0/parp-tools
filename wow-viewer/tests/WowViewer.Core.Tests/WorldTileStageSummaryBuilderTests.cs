...
public sealed class WorldTileStageSummaryBuilderTests
{
    ...
    public void Read_SyntheticRootAdt_ProducesExpectedLiquidSignals()
    {
        ...
        WorldTileStageSummary summary = WorldTileStageSummaryBuilder.Read(stream, fileSummary);

        Assert.Equal(MapFileKind.Adt, summary.Kind);
        Assert.Equal(1, summary.WdlVisibleTileCount);
        Assert.Equal(1, summary.TerrainChunkCount);
        Assert.Equal(0, summary.TerrainHoleChunkCount);
        Assert.Equal(1, summary.LiquidChunkCount);
        Assert.Equal(1, summary.LiquidLayerCount);
        Assert.Equal(4, summary.VisibleLiquidTileCount);
