...
from harvester.v19_dataset import V19Dataset  # noqa: E402
from harvester.v19_models import V19HeightModel  # noqa: E402
from harvester.v19_losses import v19_combined_loss  # noqa: E402

_PROJECT_ROOT = Path(__file__).resolve().parent.parent.parent
_DATASET_ROOT = _PROJECT_ROOT / "output" / "datasets" / "v18"
_MODELS_ROOT = _PROJECT_ROOT / "output" / "ml-training" / "v19"
_PANEL_SIZE = 256
...
def _v19_loss(
    ...
    terrain_valid_mask = batch["terrain_valid_mask"].to(device, non_blocking=True)
    normals = batch["normals"].to(device, non_blocking=True) if "normals" in batch else None
    normal_mask = batch["normal_mask"].to(device, non_blocking=True) if "normal_mask" in batch else None
    height_mean = batch["height_mean"].to(device, non_blocking=True) if "height_mean" in batch else None
    height_std = batch["height_std"].to(device, non_blocking=True) if "height_std" in batch else None

    global_height, local_height, bounds = model(inp)
    
    nc_weight = float(getattr(args, "normal_consistency_weight", 0.1))
    edge_weight = float(getattr(args, "edge_weight", 0.05))

    loss, metrics = v19_combined_loss(
        pred_global=global_height,
        pred_local=local_height,
        pred_bounds=bounds,
...
def _preview_v19(batch: dict[str, Any], outputs: dict[str, torch.Tensor], out_path: Path) -> None:
    n = min(int(batch["input"].shape[0]), 8)
    rows = []
    row_titles = []
    for idx in range(n):
        ...
        panels: list[tuple[str, torch.Tensor]] = [
            ("minimap", input_rgb),
            ("height_gt", batch["target"][idx]),
            ("height_pred_global", outputs["pred"][idx]),
            ("height_pred_local", outputs["pred_local"][idx]),
            ("terrain_valid", outputs["terrain_valid_mask"][idx]),
        ]
        if batch["input"][idx].shape[0] == 6:
            input_normals = batch["input"][idx][3:6]
            panels.insert(1, ("input_normals", _normals_to_rgb(input_normals)))
        rows.append(panels)
    _save_preview_grid(rows, out_path, row_titles=row_titles)
...
TASKS: dict[str, TaskSpec] = {
    ...
    "v19": TaskSpec("v19", lambda: None, _v19_loss, _preview_v19),
}
...
def _parse_args(task_name: str) -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        default=6,
        choices=[3, 6],
        help="Number of input channels for V19 task (3 = minimap RGB, 6 = minimap + normal XYZ)",
    )
    p.add_argument(
        "--edge-weight",
        type=float,
        default=0.05,
        help="Weight for Sobel edge loss in V19 task.",
    )
    return p.parse_args()
...
def run_task(task_name: str) -> None:
    ...
    if task_name == "v19":
        task = TaskSpec(
            name="v19",
            model_factory=lambda: V19HeightModel(in_channels=args.input_channels),
            loss_fn=_v19_loss,
            save_preview=_preview_v19,
        )
    else:
    ...
    if args.paste_dir is not None:
        ...
        try:
            ...
            print("Validation set has zero paste candidates ΓÇö using raw tile dataset")
            if task_name == "v19":
                val_ds = V19Dataset(
                    dataset_root=args.dataset_dir,
                    builds=args.builds,
