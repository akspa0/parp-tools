...
DEFAULT_OUTPUT_DIR = WORKSPACE_ROOT / "output" / "ml-training" / "v10_minimap_to_mclay"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train the v10 minimap-to-MCLY palette classifier from NPZ shards and a mined MCLY dictionary.")
    ...
    parser.add_argument("--min-retained-chunks", type=int, default=1, help="Minimum retained dictionary chunks needed to label a tile.")
...
def infer_tile_label(
    texture_ids: np.ndarray,
    texture_names: list[str],
    dictionary: dict[str, DictionaryEntry],
) -> tuple[DictionaryEntry, int, int, float] | None:
    counts: Counter[DictionaryEntry] = Counter()
    for y in range(texture_ids.shape[0]):
        for x in range(texture_ids.shape[1]):
    ...
    entry, dominant_count = counts.most_common(1)[0]
    retained_count = sum(counts.values())
    total_chunks = int(texture_ids.shape[0] * texture_ids.shape[1])
    return entry, retained_count, dominant_count, dominant_count / max(1, total_chunks)


def discover_samples(npz_paths: Iterable[Path], dictionary: dict[str, DictionaryEntry], min_retained_chunks: int, max_samples: int) -> list[MclySample]:
    samples: list[MclySample] = []
    for path in npz_paths:
        with np.load(path, allow_pickle=False) as shard:
            ...
            if texture_ids.ndim != 3 or texture_ids.shape[0] != 16 or texture_ids.shape[1] != 16:
                continue
            if minimap_rgb.shape != (256, 256, 3):
                continue

            label = infer_tile_label(texture_ids, texture_names, dictionary)
            if label is None:
                continue
            entry, retained_count, dominant_count, dominant_fraction = label
            ...
            samples.append(
                MclySample(
                    ...
                    combination_hash=entry.combination_hash,
...
class MclyDataset(Dataset[dict[str, Any]]):
    def __init__(self, samples: list[MclySample], remapped_labels: dict[int, int]):
    ...
    def __getitem__(self, index: int) -> dict[str, Any]:
        ...
        return {
            "inputs": minimap,
            "label": torch.tensor(self.remapped_labels[sample.label_index], dtype=torch.long),
...
class MinimapToMclyClassifier(nn.Module):
...
def split_samples(samples: list[MclySample], val_fraction: float, seed: int) -> tuple[list[MclySample], list[MclySample]]:
    by_label: dict[int, list[MclySample]] = {}
    for sample in samples:
        by_label.setdefault(sample.label_index, []).append(sample)

    train: list[MclySample] = []
    val: list[MclySample] = []
    ...
    for label_samples in by_label.values():
        ...
        if len(label_samples) == 1:
            ...
            continue
        val_count = max(1, min(len(label_samples) - 1, int(math.ceil(len(label_samples) * val_fraction))))
    ...
    if not train or not val:
        raise ValueError("Need enough labeled shards to create train and validation splits.")
    return train, val


def make_loader(dataset: MclyDataset, batch_size: int, shuffle: bool, num_workers: int) -> DataLoader:
    return DataLoader(
        dataset,
...
def summarize_label_distribution(samples: list[MclySample], index_to_entry: dict[int, DictionaryEntry]) -> list[dict[str, Any]]:
    ...
    return [
        ...
        for label_index, count in sorted(counts.items(), key=lambda item: (-item[1], item[0]))
    ]
...
def main() -> None:
    ...
    if len(samples) < 2:
        raise RuntimeError("Need at least two v10 NPZ shards with minimap_rgb_256 and retained mcly_texture_ids labels.")

    active_label_indexes = sorted({sample.label_index for sample in samples})
    if len(active_label_indexes) < 2:
        raise RuntimeError("Need at least two retained MCLY labels to train a classifier.")
    ...
    train_dataset = MclyDataset(train_samples, remapped_labels)
    val_dataset = MclyDataset(val_samples, remapped_labels)
    ...
    model = MinimapToMclyClassifier(class_count=len(active_label_indexes)).to(device)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        print(
            ...
            f"train acc {train_metrics['accuracy']:.3f} | val loss {val_metrics['loss']:.4f} | "
        ...
        if val_metrics["loss"] < best_val_loss:
            ...
            torch.save(checkpoint, output_dir / "minimap_to_mclay_classifier.pt")
