# Tasks: ADT v26 — First Reader and Renderer for a Brand-New Terrain Format

**Input**: `specs/archived/237-adt-v26-terrain/` (spec, plan, research, data-model, contracts/cli-contract.md, quickstart)
...
**Corpus status (2026-09-16)**: **on hand**: 700 files (699 unique) in `wow-viewer/test_data/v22_adts/unknown/` (git-ignored; `$env:WOWVIEWER_AHDR_CORPUS` overrides), extensionless and named by FileDataID. **ADT v26: a brand-new format first seen 2026-09-16 (not the wiki's v22)**; see evidence/phase0-first-look-2026-09-16.md. Tasks marked 🗂 use the real corpus.

Format: `- [ ] T### [P?] [US?] description (path)`. All paths are relative to `wow-viewer/`.

## Phase 1: Setup

- [ ] T001 Create evidence folder and README stating the receipt format (command, root, file count, SHA-256 of file list) in specs/archived/237-adt-v26-terrain/evidence/README.md
...
- [ ] T005 Grep all `AdtV23` switch/predicate sites across src/ and tools/ and extend each one explicitly for the new kinds; list the sites touched in specs/archived/237-adt-v26-terrain/evidence/t005-kind-sites.md
...
- [ ] T009 Widen the guard to all AHDR kinds in src/core/WowViewer.Core.IO/Maps/AdtV23SummaryReader.cs and print `ADT/v22 semantics:` / `ADT/v23 semantics:` / `ADT/v26 semantics:` by kind in tools/inspect/WowViewer.Tool.Inspect/Program.cs (~line 9632)
...
- [ ] T050 [US0] 🗂 Operator visual gate: screenshots, crack check, inner-grid sanity note in specs/archived/237-adt-v26-terrain/evidence/fastpath-wireframe.md
...
- [ ] T017 [US1] 🗂 Run the inventory on the real corpus, confirm it reproduces every number in evidence/phase0-first-look-2026-09-16.md (699 revision-26 files, 0 unaccounted bytes, chunk table, ALOC tiles), and write the gate receipt, turning every unknown chunk into a named research item in specs/archived/237-adt-v26-terrain/evidence/phase0-corpus-inventory.md and specs/archived/237-adt-v26-terrain/research.md
...
- [ ] T026 [US2] 🗂 Run layout-probe on the corpus, record winners and margins, and fix the reader to the winning layouts in specs/archived/237-adt-v26-terrain/evidence/phase1-layout-probe.md and src/core/WowViewer.Core.IO/Maps/AdtAhdrReader.cs
...
- [ ] T032 [US2] Extend the living format document (created 2026-09-16 with the first-look facts) with every Phase 1 measurement, promoting hypotheses only with linked evidence, in docs/architecture/adt-v26-format.md
- [ ] T033 [US2] 🗂 Phase 1 gate receipt with SC-002–SC-006 numbers in specs/archived/237-adt-v26-terrain/evidence/phase1-decode.md
...
**Depends on**: US2 gate, and Spec 238 local CASC reads (US1, T014) + viewer data source (T034) for real textures/models. Without them, layers render as index colours and placements as labelled markers.

- [ ] T051 [US3] Resolve `ATEX`/`ADOO` names to FileDataIDs via the community listfile (refresh with the existing ListfileDownloader), then load BLP/M2/WMO bytes through the Spec 238 CASC data source, with index-colour/marker fallback, in src/viewer/WoWViewer/Terrain/AhdrTerrainAdapter.cs
- [ ] T052 [P] [US3] Re-run evidence/scripts/names_to_fdid_v26.py against the refreshed listfile and record coverage in specs/archived/237-adt-v26-terrain/evidence/asset-resolution.md
...
- [ ] T039 [US3] "Open ADT v26 folder…" entry beside the Rosetta datastore open (~ViewerApp.cs:13017), wired through TerrainManager, in src/viewer/WoWViewer/ViewerApp.cs and src/viewer/WoWViewer/Terrain/TerrainManager.cs
- [ ] T040 [US3] 🗂 Operator visual gate with screenshots (seams, blending, objects on ground) in specs/archived/237-adt-v26-terrain/evidence/phase2-render.md

## Phase 6: User Story 4 — Existing formats unaffected (P2)

- [ ] T044 [P] Investigate the unexplained revision-26 values (ALOC[0]=2869, ALOC fields 3/4, AOCH all-zero, ADST, AHDR+0x14 = 8396383) and record the results in specs/archived/237-adt-v26-terrain/research.md R14
- [ ] T041 [US4] Run the full test suite and open one 0.5.3 Alpha map and one 3.3.5 LK map; record results in specs/archived/237-adt-v26-terrain/evidence/phase2-regression.md

## Phase 7: Polish

- [ ] T042 [P] Document `adt-ahdr` commands, then diff docs/CLI-TOOLS.md and specs/archived/237-adt-v26-terrain/quickstart.md against the real argument parser
