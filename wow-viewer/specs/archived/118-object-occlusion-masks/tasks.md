...
**Input**: Design documents from `/specs/archived/118-object-occlusion-masks/`
...
- [x] T028 [P] Verify `specs/archived/118-object-occlusion-masks/contracts/cli-contract.md` and `quickstart.md` against the REAL argparse of every new/changed CLI (this project's "verify docs against real argparse" convention — Spec 116 lesson: passing focused tests only proves library functions, not that documented commands parse)
