...
> **ARCHIVED 2026-09-23 — open residue folded into an epic.** Phase 5 MoP to 248; Phases 1-4 workspace/editor mode to 251. Successor: [Epic 248](../../248-epic-formats-and-conversion/spec.md). Status lines and checkboxes below are historical and were audited against the code ([audit](../reconciliation-2026-09-23/audit/batch-D1.md)); they are not implementation authority.
...
5. **4.3.4 through 5.1 MoP ADT & Rendering Modernization Roadmap**: Expands `StandardTerrainAdapter` and `TerrainRenderer` to support split ADT secondary streams (`_obj0`/`_obj1`, `_tex0`/`_tex1`), height texture blending (`MHID`, `MDID`, `MCXH`), and WMO terrain vertex blending based on binary decompilation of `WoW.exe` 5.0.1.15464 via Ghidra. The conversion workflow separately selects a serialization target: Alpha 0.5.3 monolithic WDT, LK v18 monolithic ADT, or (once implemented) the native Cataclysm/MoP split ADT family.
...
- **AC-006 (4.3.4–5.1 MoP ADT reader/runtime pipeline)**: `StandardTerrainAdapter` discovers and routes the native-aligned 5.0.1 root plus selected `_obj0`/`_obj1` and `_tex0`/`_tex1` companions, parses companion `MCNK` payloads without the root-only 128-byte header, and preserves sparse physical MCIN slot identity in the supported reader/runtime paths. `MHID`, `MDID`, and `MCXH` remain recognized but evidence-gated; exact native height/shader and WMO seam parity is not claimed by this criterion.
- **AC-008 (explicit map serialization target)**: The map-converter UI exposes an explicit source-family/target-format pair and reports loss. Alpha identity output and LK v18 monolithic output are routed to their target-specific writers; split-to-Alpha and split-to-LK are explicitly lossy down-conversions. The LK writer cannot be selected for native MoP split output.
- **AC-009 (native MoP writer safety)**: Cataclysm/MoP split output remains disabled until a real split-family writer exists that preserves independent root/object/texture bands, 256 physical MCIN slots, root-only MCNK headers, and modern split-only state. No monolithic LK result may be labeled as native MoP split output.
