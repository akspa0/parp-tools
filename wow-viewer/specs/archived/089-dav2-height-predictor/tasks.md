...
- [x] T035 [US4] Run on a real local 12 GB class GPU for 2 epochs on a 16-tile real V22 subset. Confirm zero CUDA OOM with the 12 GB profile (`--device cuda --target-vram-gb 12 --memory-profile 12gb --batch-size 1 --grad-accum-steps 4 --gpct-K 2 --gpct-weight 0.1 --bias-free-mask-ratio 0.15`). Capture peak VRAM via `torch.cuda.max_memory_allocated() / 1e9` and record it in the run folder's `peak_vram.json`. Do this before any future remote/RunPod retry.

2026-07-04 status note: T031-T034 are validated locally, and the trainer now has a real 12 GB profile instead of a fake `target-vram-gb` placeholder. `train_v23_height.py` now applies memory profiles, honors `grad_accum_steps`, records `peak_vram.json`, and retries by shrinking batch size / GPCT-K / AMP mode on CUDA OOM. Focused local coverage for the profile logic lives in `tests/v23/test_train_profiles.py`.

2026-07-04 follow-up: T035 is now validated locally on an RTX 4070 Ti SUPER using `3_3_5_12340.zarr`, 16 train tiles, 4 val tiles, `--memory-profile 12gb`, `--batch-size 1`, `--grad-accum-steps 4`, `--gpct-K 2`, and `--bias-free-mask-ratio 0.15`. Run folder: `wow-viewer/models/v23/height/runs/t035_local_12gb_20260704/`. Artifacts: `peak_vram.json`, `metrics.json`, `checkpoints/v23_height_{last,best}.pt`, `val_preview_1/tile_0.png`, `val_preview_2/tile_0.png`. Peak allocated VRAM: `0.408541184 GB`; peak reserved VRAM: `0.457179136 GB`; zero CUDA OOM. Caveat: the HF DA-V2-Small checkpoint was not cached locally, so the encoder fell back to random DepthAnything initialization. This proof validates the real CUDA training envelope and artifact capture, not pretrained DA-V2 quality.

2026-07-05 follow-up: V23 training now supports `--maps` so local proof can target critical map knowledge instead of taking the first N store rows. `3_3_5_12340` V18 and V22 both contain `Northrend` with 1,131 tiles. A Northrend-specific local smoke passed: `t035_northrend_local_12gb_20260705`, command added `--maps Northrend`, checkpoint config records `maps = ["Northrend"]`, zero CUDA OOM, peak allocated VRAM `0.408541184 GB`, peak reserved VRAM `0.455081984 GB`. Same HF-cache caveat applies.

2026-07-05 correction: the local proof route must use the V18 curation manifest, not just V22 first-N or map-only filtering. `train_v23_height.py` now accepts `--curation-manifest` plus the V18 thresholds, filters V22 samples through `harvester.v16_curation.load_curation_index`, selects validation from highest mismatch/difficulty curated rows first, and writes labeled validation previews. Corrected curated smoke passed: `v23_curated_northrend_labeled_smoke_20260705`, `--curation-manifest ../output/datasets/v18/curation/v18_focus_terrain_all_v1/kept_tiles.parquet`, `--maps Northrend`, zero CUDA OOM, peak allocated VRAM `0.3959296 GB`, preview `val_preview_1/tile_0.png` includes panel labels and map/tile/bucket/mismatch text. Same HF-cache caveat applies.

2026-07-05 operator-surface correction: the trainer must report learning progress as plain loss values, not only component names or VRAM artifacts. `train_v23_height.py` now prints `loss=...` for train/val batches, `train_loss=...`, `val_loss=...`, and `best_val_loss=...` at epoch summary, and writes `loss_history.jsonl` with batch and epoch JSON records. `peak_vram.json` remains capacity evidence only. Focused smoke asserts the console and JSONL trace exist, and the full V23 suite passed with `35 passed, 14 warnings`: `uv run python -m pytest tests/v23 -m v23 -q`.

2026-07-05 autotune correction: V23 now has a real startup batch autotune path matching the earlier V16 operator pattern. `--autotune-batch-size` probes `--autotune-batch-candidates` on CUDA, selects the largest candidate whose reserved memory fits under `--target-vram-gb * --autotune-safety-factor`, writes `batch_autotune.json`, and rebuilds train/val loaders with the selected batch size. CPU runs write a skipped evidence file. Focused profile/smoke tests passed with `6 passed, 14 warnings`; full V23 suite passed with `36 passed, 14 warnings`.

2026-07-05 validation-cadence correction: `--val-interval` is now honored. Validation is forward-only measurement, not training, and skipped epochs save `v23_height_last.pt` without writing a fake validation score or best checkpoint. The final epoch still validates when `--val-interval > 0`, so the 2-epoch local command uses `--val-interval 2 --val-preview-interval 2` to avoid a measurement pass immediately after the long first epoch. Focused profile/smoke tests passed with `8 passed, 14 warnings`; full V23 suite passed with `38 passed, 14 warnings`.
...
- [x] T044 [P] [US6] Implement `runpod/v23/train.sh` — curated key-map default wrapper over `train_v23_height.py`. With explicit args it remains a raw pass-through. With no args it requires packaged `config/tileset_prune_table.json` and `config/curation_manifest.parquet`, then runs the V18-curated 2K key-map corpus (`0_5_3_3368`, `3_3_5_12340`; `Azeroth Kalimdor Kalidar PVPZone01 PVPZone02 Northrend Expansion01`) with visible per-step logging, startup batch autotune, `--val-interval 2`, GPCT-K 2, SDC enabled, and bias-free masking.
...
- [ ] T048 Ship the V23 bundle to a 24 GB RunPod Pod per Phase 7. For the next stability proof, package the V18 curation manifest and run `bash runpod/v23/train.sh` with no positional args; defaults are the curated 2K key-map corpus, 24 GB memory profile, startup batch autotune candidates `1 2 4 8 12 16 24 32 40 48 64 80 96`, `--log-interval 1`, and `--val-interval 2`. For later full-corpus training, override env vars such as `TRAIN_MAX_TILES=25000 EPOCHS=4 VAL_INTERVAL=1 RUN_NAME=v23_height_full_corpus_v1 bash runpod/v23/train.sh`. Capture peak VRAM via `nvidia-smi dmon` sampling or by reading the trainer's CUDA memory log.
