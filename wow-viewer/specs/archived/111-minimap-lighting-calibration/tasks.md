...
- [ ] T009 [US1] Code proof complete: focused C# sweep (`MinimapShadingMatchTests` + `MinimapLightingProvenanceTests` + `TerrainMinimapCompositorTests` + `TerrainSolarDirectionTests`) 42/42; Debug Harvest build 0 errors; `tests/spec111/` Python suite 16/16. **Remaining user-run proof**: one bounded real-0.5.3.3368 harvester pass (full-texture-decode profile -- transport only; the v50 lane is the destination and its store pass waits on Spec 109's builder per `research.md`), the quickstart side-by-side eyeball check on a handful of `matched` tiles, then the whole-build pass and reconciled distribution report
...
- [x] T016 [US3] Confirmed at implementation time (2026-07-17, corrected same day after user feedback): the active lane is **v50** -- the canonical RGB-to-WDL trainer entry is `scripts/v50_train_wdl_prior.py` (default `--release v50.1`, enforces `require_store_release`); Spec 102 remains BLOCKED on its M0 target reharvest. No baseline checkpoint is committed to the repo -- it is a user-run artifact, so `--baseline-checkpoint` is a required explicit path that fails closed when absent
- [x] T017 [US3] Implement `data-harvester/scripts/train_spec111_reconstruction.py`: validates the rebalanced plan (bare-float lighting_times, leak-safety assertion, existing store/baseline), prints the exact delegated canonical `v50_train_wdl_prior.py` command with `--release` passthrough, and refuses to start any GPU run without `--confirm-run`
...
- [x] T021 Update task states and exact proof commands in `specs/archived/111-minimap-lighting-calibration/tasks.md` as each phase's checkpoint is reached
