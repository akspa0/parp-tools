...
**Input**: Design documents from `specs/archived/114-direct-terrain-reconstruction/`
...
- [ ] T001 Record the completed Spec 113 T010b fixed-noon-white visual evidence and fresh synthetic store/manifest hashes in `specs/archived/114-direct-terrain-reconstruction/research.md`
- [x] T002 Audit Kalimdor/Azeroth v50.1 signal coverage and confirm numeric height/normal/liquid/material/alpha plus authored RGB were not invalidated by the synthetic-lighting fix in `specs/archived/114-direct-terrain-reconstruction/research.md`
- [x] T003 Freeze the strongest completed Spec 112 run identity and metrics as `direct_cnn_v112` comparison evidence in `specs/archived/114-direct-terrain-reconstruction/research.md`
...
- [ ] T010 Run CPU fixtures and hand the user the exact real corpus-build command, output paths, coverage expectations, and time/disk estimate in `specs/archived/114-direct-terrain-reconstruction/quickstart.md`
...
- [x] T014 [US1] Add the compact `mit_b0_regression` architecture with one continuous relative-height output in `data-harvester/src/harvester/v50/direct_geometry_model.py`
- [x] T015 [US1] Implement same-split training/evaluation, flat/tile-mean baselines, border metrics, and held-out visual sheets in `data-harvester/src/harvester/v50/direct_geometry_train.py`
...
- [x] T017 [US1] USER RUN: train tonight's authored-only `direct_cnn_v112` baseline with the exact proven command in `specs/archived/114-direct-terrain-reconstruction/quickstart.md`; corrected dual-view and `mit_b0_regression` runs follow after T014/T015 and the Spec 113 rerender gate — **DONE 2026-07-19: run completed 100 epochs and FAILED SC-001 (best epoch 92, val MAE 0.149267 vs tile-mean 0.138747); immutable negative evidence, do not rerun this recipe**
- [x] T018 [US1] Validate the authored baseline summary first, then compare the later corrected dual-view/CNN/MiT runs against SC-001/SC-002 and record the user's visual verdict in `specs/archived/114-direct-terrain-reconstruction/research.md` — **baseline validated and recorded in research.md; dual-view/CNN/MiT comparison remains open pending T014/T015 and the Spec 113 rerender gate**
- [x] T056 [US1] Add the deployment inference path FR-015 requires: loose 256×256 minimap tile(s) → 16-bit relative-relief PNG(s) + side-by-side review sheet + per-tile auditable manifest in `data-harvester/src/harvester/v50/direct_geometry_infer.py` and `data-harvester/scripts/v50_infer_direct_geometry.py` (ID assigned in creation order; placement is Phase 3)
- [x] T057 [US1] USER RUN: train the spectral-guidance ablation (`--spectral-weight 0.1 --multiscale-weight 0.25`) as `mit_b0-authored-v2-spectral` and compare against the frozen `mit_b0-authored-v1` metrics per the research.md spectral-bias record — **DONE 2026-07-20: best epoch 130, val MAE 0.193435, SC-001 false, SC-002 true; visually sharper structure, numeric plateau ≈0.19 confirmed; motivated the residual detailer decision in research.md**
- [x] T058 [US1] Implement coarse-output materialization (frozen checkpoint → derived `coarse_relief` Zarr bound to checkpoint hash; source stores immutable) in `data-harvester/src/harvester/v50/direct_geometry_materialize.py` and `data-harvester/scripts/v50_materialize_coarse_relief.py`
- [x] T059 [US1] Implement the residual detailer architecture (RGB + generated coarse → one residual field, final = coarse + residual) in `data-harvester/src/harvester/v50/geometry_detailer_model.py`
- [x] T060 [US1] Implement the detailer trainer with the coarse-only strong baseline, ≥5% gate, SC-002, sheets, and `upstream_models` provenance in `data-harvester/src/harvester/v50/geometry_detailer_train.py` and `data-harvester/scripts/v50_train_geometry_detailer.py`
- [x] T061 [US1] USER RUN: materialize the frozen coarse checkpoint's outputs, then train/evaluate the detailer against the coarse-only baseline and record the verdict in `specs/archived/114-direct-terrain-reconstruction/research.md` — **DONE 2026-07-20: best epoch 91, val MAE 0.170665 vs coarse-only 0.187800 (9.1% relative), gate=True, sc002=True, promotion=pending(user visual gate); first Spec 114 geometry checkpoint to clear its numeric gate; user visual verdict positive**
- [x] T062 [US1] Compare the old V7 model's multi-frequency band approach (April 2026) against the current single-residual detailer and record the structural-prior findings in `specs/114-direct-terrain-research/research.md` — **DONE 2026-07-20: V7 had 4 structural terms the current detailer lacks (full 2D frequency, Laplacian curvature, Sobel edge, transition-focus 3×); V25 had explicit LF/HF band split; decision is to port all five as loss-only flags into the detailer trainer (T063)**
- [x] T063 [US1] Port V7's multi-frequency structural prior (full 2D frequency, Laplacian, Sobel edge, transition-focus) + V25's LF/HF band split into `spectral_guidance.py` as loss-only terms, wired behind flags in `geometry_detailer_train.py` (default 0 = parity); 17 spectral tests + 16 detailer tests pass — **DONE 2026-07-20: user-run `detailer-mit_b0-authored-v2-bandsplit` best epoch 89, val MAE 0.170494 vs coarse-only 0.187800 (9.2% relative), gate=True, sc002=True; user visual verdict: "visually stunning, very very close" — the band-split loss stack produced a significant visual quality jump over v1 despite a small numeric improvement (0.170665 → 0.170494); bf16 AMP, val-tolerance 0.01, liquid-mask-weight 0.5, per-epoch validation previews all active**
- [x] T063-continued [US1] Resumed from epoch 89 checkpoint with `--init-weights` for 200 epochs — **DONE 2026-07-20: best epoch 164, val MAE 0.166769 vs coarse-only 0.187800 (11.2% relative), gate=True, sc002=True; plateaued at epoch 164 after 36 epochs of no improvement; 100 additional epochs yielded 0.003725 absolute MAE improvement over the 100-epoch run**
...
- [ ] T019 [US2] Audit existing verified placement/runtime geometry and freeze the bounded top-down visibility export seam in `specs/archived/114-direct-terrain-reconstruction/research.md`
...
- [ ] T026 [US2] USER RUN: render labels, train the mask model, and persist generated masks for the frozen geometry split using commands documented in `specs/archived/114-direct-terrain-reconstruction/quickstart.md`
- [ ] T027 [US2] Retrain/evaluate geometry with generated masks only and record the SC-003 promotion verdict in `specs/archived/114-direct-terrain-reconstruction/research.md`
...
- [ ] T034 [US3] USER RUN: build the family-safe library/corpus and train the feature classifier with commands documented in `specs/archived/114-direct-terrain-reconstruction/quickstart.md`
- [ ] T035 [US3] Record per-class coverage, macro-F1, unknowns, overlays, and the SC-004 verdict in `specs/archived/114-direct-terrain-reconstruction/research.md`
...
- [ ] T042 [US4] USER RUN: build the canonical family corpus and train the selector using commands documented in `specs/archived/114-direct-terrain-reconstruction/quickstart.md`
- [ ] T043 [US4] Record majority comparison, family-safe audit, unknown coverage, recomposition review, and SC-005 verdict in `specs/archived/114-direct-terrain-reconstruction/research.md`
...
- [ ] T050 [US4] USER RUN: train/evaluate alpha reconstruction with generated family selections using commands documented in `specs/archived/114-direct-terrain-reconstruction/quickstart.md`
- [ ] T051 [US4] Record blend-field metrics, recomposition metrics, visual review, and SC-006 verdict in `specs/archived/114-direct-terrain-reconstruction/research.md`
...
- [ ] T052 [P] Audit authored-minimap-to-output lineage and prove zero ground-truth inference inputs for SC-007 in `specs/archived/114-direct-terrain-reconstruction/research.md`
- [ ] T053 [P] Prove independent checkpoint replacement without unrelated retraining for SC-008 in `specs/archived/114-direct-terrain-reconstruction/research.md`
...
- [ ] T055 Run focused Python/C# tests, schema validation, `git diff --check`, and the final user visual gate; record exact proof in `specs/archived/114-direct-terrain-reconstruction/quickstart.md`
