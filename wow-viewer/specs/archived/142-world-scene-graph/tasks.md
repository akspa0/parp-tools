...
**Input**: Design documents from `/wow-viewer/specs/archived/142-world-scene-graph/`
...
- [x] T001 Confirm `WowViewer.Core.Runtime` and `WowViewer.Core.Tests` are the owners for the graph contract in `wow-viewer/specs/archived/142-world-scene-graph/plan.md`.
- [x] T002 [P] Add the synthetic workload schema and replay rules to `wow-viewer/specs/archived/142-world-scene-graph/contracts/synthetic-world-workload.schema.md`.
...
- [ ] T013 [US6] Add a dry-run serialization/replay example and explicit non-renderer-benchmark warning to `wow-viewer/specs/archived/142-world-scene-graph/quickstart.md`.
...
- [x] T059 [US2] Build the validation-capture seam and hand off the owner-attribution capture from
  `specs/archived/142-world-scene-graph/phase-8j-overlay-recovery.md`; record only user-run evidence.
...
- [ ] T029 Run the focused and full test commands from `wow-viewer/specs/archived/142-world-scene-graph/quickstart.md` and record evidence in `wow-viewer/memory-bank/progress.md`.
...
- [x] T068 [US2] Expose active-tile and detailed-terrain-draw diagnostics at the terrain render
  boundary, with the invariant widened to the configured detail ceiling.
- [x] T068a [US2] Preserve the renderer's established ADT coordinate span when widening the
  selector; `WoWConstants.ChunkSize` is the 533.333-yard ADT span used by terrain/object placement,
  while `WoWConstants.TileSize` is not a valid replacement for camera tile admission.
- [ ] T069 [US2] Run a user-owned real-client movement capture proving the selected count follows
  the ADT detail slider and report active, retained, loaded, and detailed draw counts separately.
- [ ] T070 [US2] Tune the next bounded FOV-radiation slice only from real-client evidence; it must
  remain inside the directional cone and must not restore radial or map-wide admission.
...
- T066-T068a establish the bounded spatial baseline before any future FOV-radiation or dense
