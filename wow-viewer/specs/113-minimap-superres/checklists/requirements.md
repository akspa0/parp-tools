# Specification Quality Checklist: Minimap Super-Resolution (Real-ESRGAN)
...
- "Real-ESRGAN" is named as the model family per the user's explicit request; it is a scope anchor,
  not an implementation leak — the exact architecture/loss/pretrained-init choices are deferred to
  the plan. Domain artifact names (`minimap_rgb_authored`, `minimap_rgb_1024`, MCAL/MCNR, per-build
...
- US1 is deliberately a hard gate: unlike a normal MVP it can *fail* (authored↔detail misalignment
  beyond correction), in which case the spec halts and surfaces the finding rather than proceeding
  to train on invalid pairs. This is called out in Edge Cases and SC-002.
