...
- `dotnet test wow-viewer/tests/WowViewer.Core.Tests/WowViewer.Core.Tests.csproj -c Debug --no-restore --filter "FullyQualifiedName~WdtSummaryReaderTests|FullyQualifiedName~WmoAdmissionTallyTests|FullyQualifiedName~WorldObjectVisibilityCollectorTests"` — **passed: 29/29**. Covers WDT WMO-based classification and generic WMO visibility/admission policy, not the GL-backed `WorldScene` path.
