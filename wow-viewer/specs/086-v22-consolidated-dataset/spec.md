...
As a dataset builder, I want a single C# harvest/build command that produces every signal downstream needs, so I never run a post-build patch script again.
...
As a downstream consumer, I want one fixed C# V22 dataset contract that produces every signal I need without fallback chains or optional `has_*` guards.
...
**Independent Test**: the C# V22 reader returns the same keys for every tile. Signal arrays are never absent.

**Acceptance Scenarios**:

1. **Given** a V22 C# dataset store with 100+ tiles,
    **When** the C# V22 reader is initialized with no special flags,
...
**Independent Test**: the C# V22 reader returns `mddf_placement_data` and `modf_placement_data` arrays for every tile with correct shapes and dtypes.

**Acceptance Scenarios**:

1. **Given** a V22 C# dataset store,
...
**Independent Test**: the C# V22 reader accepts an available-signal list and returns zero-filled arrays for any requested signal not yet built.
...
- **FR-006**: The C# V22 reader MUST return a fixed set of batch keys with no conditional `has_*` guards. Missing per-tile signals produce zero-filled arrays; missing per-build model or tileset entries produce empty arrays with `load_error` flag.
...
- **FR-012A**: V22 MUST preserve each tile's MTEX texture path table as `mtex_texture_paths` (or an equivalent C# field) so every `mcly_texture_ids` value can resolve to the exact source texture path before build-wide tileset remapping.
...
- **FR-014**: Tileset textures MUST be decoded by the C# harvester from BLP files in the MPQ archives — one decode per unique path, cached and re-emitted per tile reference.
...
V22 stores the **complete** parsed model document for every unique M2 and WMO, not just extracted vertices — so masking models have everything the C# harvester uses: render flags, blend modes, textures, bone lookups, and all 3D geometry.
...
- **FR-019**: Each WMO model entry MUST contain:
  ...
  - `version` (uint32)
- **FR-020**: Model extraction MUST happen in the C# harvester during preprocessing. Models are read from MPQ archives and parsed via `M2GeometryReader` + `M2SkinReader` (M2) or `WmoRenderDocumentReader` (WMO), then handed off to the Python Zarr writer as decoded data. Python must not reparse the game client to rebuild them.
- **FR-021**: The C# harvester MUST de-duplicate models by canonical path (`M2ModelIdentity.FromPath`) — same model path referenced by 100 placements produces one entry.
...
- **FR-025**: The C# V22 tile record MUST include: all 20 V18 signal arrays, `mcnr_mask_257`, `liquid_type_256`, `ground_intent_height_257`, `mddf_placement_data`, `modf_placement_data`, `mddf_count`, `modf_count`, `mddf_unique_ids`, `modf_unique_ids`, `mddf_model_ids`, `modf_model_ids`, `mcly_tileset_ids`.
- **FR-026**: The C# V22 dataset contract MUST expose model entries addressable as `{model_id: {vertices, triangles, normals, bounds}}`.
- **FR-027**: The C# V22 dataset contract MUST expose tileset entries addressable as `{tileset_id: texture_rgb}`.
...
- **C# harvester** (`WowViewer.Tool.Harvest`): Already reads M2+skin for geometry + mask rasterization (spec 001), WMO for mask rasterization (`TryPaintWmoFootprint`), and BLP textures for synthetic minimap. V22 adds: (1) final V22 tile signal emission, (2) full `M2GeometryDocument` + `M2SkinDocument` emission as structured arrays in the dataset stream/store, (3) full `WmoRenderDocument` emission, (4) decoded BLP RGB for all unique terrain textures.
- **C# V22 builder/reader**: Accumulates model entries (full parsed M2/WMO), tileset entries (decoded BLP RGB), placement arrays, and all signal arrays per build. No `patch-*` subcommands and no Python post-build processing.

## Success Criteria

- **SC-001**: V22 store for 3_3_5_12340 Azeroth builds in one pass with zero patches.
- **SC-002**: The C# V22 reader returns identical batch keys for every tile — no conditional None.
...
- The canonical store format is a Zarr dataset written/read with the Python package. Transitional C# raw stream records are allowed only as a builder seam, not as the final consumer-facing cache contract.
