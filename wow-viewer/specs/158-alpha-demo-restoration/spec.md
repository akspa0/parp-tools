...
`SandBox.wtf`, or a `WTF\Account\<name>\...\config-cache.wtf` / `bindings-cache.wtf` (several clearly
generated by this project's own private-server testing, per their "WoWCore SandBox" folder names). **No
file containing worldport, teleport, or camera-placement content exists in any client this project
currently has staged.** This is a direct, exhaustive, just-measured fact — checked by content and across
the full library, not a filename-literal miss — and it changes what "restore all the demo functionality"
can mean today.
