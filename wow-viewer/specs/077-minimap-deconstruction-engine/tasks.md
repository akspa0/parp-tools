...
- [ ] T006 [P] [US1] Add shared-library tests for `ObjectLibraryEntry` and `ObjectCaptureVariant` serialization under `wow-viewer/tests/WowViewer.Core.Tests/`.
...
- [ ] T008 [US1] Add shared data contracts for object-library entries and capture variants under `wow-viewer/src/core/WowViewer.Core/Maps/` or a nearby canonical owner.
- [ ] T009 [US1] Add object-library Zarr/Parquet writer support under `wow-viewer/src/core/WowViewer.Core.IO/Maps/`.
...
- [ ] T011 [US1] Add capture-job enumeration from harvested placement name tables under `wow-viewer/data-harvester/scripts/` or a thin CLI wrapper, without inventing a parallel metadata source.
- [ ] T012 [US1] Persist captured image, mask, capture status, and metadata rows into `wow-viewer/output/datasets/object-library/`.
- [ ] T013 [US1] Emit bounded review artifacts for captured objects under `wow-viewer/output/analysis/object-library/`.
...
- [ ] T014 [P] [US2] Add pytest coverage for no-object pass-through and object-heavy suppression behavior in `wow-viewer/data-harvester/tests/`.
- [ ] T015 [P] [US2] Add tests for metadata/index parity between source V18 tiles and generated teacher-prior rows.

### Implementation for User Story 2

- [ ] T016 [US2] Add a Python library module for teacher-prior generation under `wow-viewer/data-harvester/src/harvester/`.
- [ ] T017 [US2] Add a CLI script under `wow-viewer/data-harvester/scripts/` that reads V18 Zarr stores and writes the teacher-prior dataset.
- [ ] T018 [US2] Make the prior generator explicitly prefer filtered precise/object-filtered masks over coarse rectangle roof masks where available.
- [ ] T019 [US2] Add processed-prior review-artifact rendering under `wow-viewer/data-harvester/scripts/`.
- [ ] T020 [US2] Document the exact phase-1 prior channels in the generated dataset metadata.
...
- [ ] T022 [P] [US3] Add dataset-loader tests for the new processed-prior height dataset in `wow-viewer/data-harvester/tests/`.
- [ ] T023 [P] [US3] Add a trainer smoke test or deterministic dry-run assertion for the height-only lane.

### Implementation for User Story 3

- [ ] T024 [US3] Audit the smallest existing height model/trainer owner to reuse under `wow-viewer/data-harvester/src/harvester/` and `scripts/`.
- [ ] T025 [US3] Add a processed-prior height dataset loader under `wow-viewer/data-harvester/src/harvester/`.
- [ ] T026 [US3] Add or adapt a height-only training script under `wow-viewer/data-harvester/scripts/` that predicts only `height_257`.
- [ ] T027 [US3] Preserve existing filtered terrain-valid weighting and authoritative raw height targets in the new lane.
- [ ] T028 [US3] Emit preview artifacts showing processed prior, prediction, and ground truth.
...
- [ ] T030 [P] [US4] Add pytest coverage for minimap-only object-mask dataset generation under `wow-viewer/data-harvester/tests/`.
- [ ] T031 [P] [US4] Add tests for asset-candidate matching output structure and confidence sorting.

### Implementation for User Story 4

- [ ] T032 [US4] Define the minimap-only object-mask output contract in a Python module under `wow-viewer/data-harvester/src/harvester/`.
- [ ] T033 [US4] Define the asset-candidate matching/classification contract keyed to the object library.
- [ ] T034 [US4] Add the smallest viable object-mask training/inference lane under `wow-viewer/data-harvester/scripts/`.
- [ ] T035 [US4] Add the smallest viable asset-candidate lane under `wow-viewer/data-harvester/scripts/`.
- [ ] T036 [US4] Restrict first pose outputs to XY and yaw in the inference contracts and review artifacts.
- [ ] T037 [US4] Generate processed priors from minimap-only object predictions.
...
- [ ] T039 [P] [US5] Add tests for deterministic height-to-normal derivation utilities.
- [ ] T040 [P] [US5] Add tests for any separate normal-refinement dataset contract if that phase is opened.

### Implementation for User Story 5

- [ ] T041 [US5] Add a documented analytic-normal baseline path for height predictions.
- [ ] T042 [US5] Decide whether analytic normals are sufficient before any model work begins.
