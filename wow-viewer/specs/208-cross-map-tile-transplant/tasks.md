...
- [ ] **T001** Audit spec 195's `ChunkTranspositionService.ExtractPayload` / `TransformPayload` /
...
- [ ] **T002** Verify 195's rotate/mirror actually transforms **normals and placement rotations**, not
      only vertex positions. FR-005 depends on it. If it does not, that is a defect in 195 to fix
      there, not to work around here.
- [ ] **T003** Map `PhaseDataChannel` onto `ChunkTranspositionOptions` (FR-003). One mechanism, one
      direction of derivation. **Do not add a third channel model.**
- [ ] **T004** Confirm 195's chunk-granular offset satisfies the operator's "partial tile offsets"
...
- [ ] **T005** Establish whether a transplant applies to the live loaded map, to a staged artifact, or
