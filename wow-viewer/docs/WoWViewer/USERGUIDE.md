...
**From source:**
```
cd gillijimproject_refactor/src/WoWViewer
dotnet run --project .\WoWViewer.csproj
```
...
### Loading ML Dataset in WoWViewer

WoWViewer can load ML datasets directly for visualization:
```
WoWViewer.exe path/to/mk_dataset/
```
...
WoWViewer can export loaded models to **glTF Binary (.glb)** format for use in external 3D tools:
