# WoWViewer — WoW World Viewer

WoWViewer is the active viewer/debugging application in this repo. It is no longer just a standalone MDX/WMO viewer. It acts as a combined world viewer, PM4 inspection surface, terrain debugger, asset browser, and export front end.
...
```powershell
dotnet build .\src\WoWViewer\WoWViewer.sln -c Debug
dotnet run --project .\src\WoWViewer\WoWViewer.csproj
```
...
- `MSHD`
  ...
  - The active confidence report still treats the broader `MSHD` semantic model as unresolved, and the active `WoWViewer` PM4 path does not use `MSHD` to drive overlay placement or match scoring.
