...
| `audio-engine-plan-2026-04-21.md` | partial | `AlphaAreaAudioCatalog*.cs` files and the `audio alpha-area` inspect path now exist, including loose/archive asset probing. MCSE reader, MIDI/DLS playback, audio runtime, audio backend, and `Core.Runtime` audio namespace remain missing. | Keep open. Alpha catalog proof is documented; next: MCSE reader and runtime integration. |
...
| `multi-model-terrain-reconstruction-2026-05-16.md` | stale | Historical minimap-domain plan. Spec 114 now requires universal raster-to-relief geometry before optional WoW object cleanup. | Keep as historical rationale; route active work to `universal-image-to-terrain-plan-2026-07-19.md`. |
...
| `universal-image-to-terrain-plan-2026-07-19.md` | active | Spec 114 arbitrary-raster relief/mesh contract, pinned teacher/student, leak-safe curriculum, observable trainer, and textured OBJ inference are implemented; failed WoW-only checkpoint remains rejected. | Build four real non-WoW family label stores and the universal curriculum, then user-run the documented dry plan and CUDA training command. |
...
9. **Spec 009 is the master design reference**: 2,650-line comprehensive specification covering every subsystem with byte-level detail, algorithms, shader source, and edge cases. Sufficient for full from-scratch reimplementation.
