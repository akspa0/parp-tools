...
- **OQ-1**: The 1.12.1 0x101 "extra-1" table at header `0xdc/0xe0` is the largest single record in the file (0x1f8 per record, 29 nested sub-tables). It is not present in 0x100. Best current guess: it is the consolidated per-batch / per-submesh record that 3.3.5 splits into multiple smaller tables. Needs a real 0x101 file to confirm.
- **OQ-2**: The 1.12.1 `*(byte *)(iVar10 + 4)` flag word controls blend modes 0x100 / 0x200 / 0x400 / 0x800 / 0x1000 / 0x2000 / 0x4000 / 0x8000 / 0x20000 / 0x40000. The flag layout overlaps with 3.3.5 but the bit meanings are likely different. Needs a real-data test to map to canonical blend modes.
- **OQ-3**: The 1.12.1 vertex table uses 12-byte entries (0xc/entry) instead of 3.3.5's interleaved 0x30-0x40. The two tables are (presumably) positions and normals, but the exact ordering and whether UVs are inside either is open. Needs a real 1.12.1 file opened in a known-good tool to compare.
- **OQ-4**: The 1.12.1 light "record" at 0xc/entry is suspicious — that's only 3 floats, not enough for color + position + attenuation. It's likely that 1.12.1 stores lights as a (position, radius) pair and the rest of light state is runtime-default. Needs confirmation.
- **OQ-5**: The 1.12.1 M2 cvar bit mapping (0x1 / 0x2 / 0x4 / 0x8 / 0x10 / 0x20 / 0x40 / 0x80 / 0x100) is not yet decompiled. The 3.3.5 mapping should be re-validated against the 1.12.1 binary, not assumed.
- **OQ-6**: How 1.12.1 handles external `.anim` files. The 3.3.5 native pass uses `%04d-%02d.anim` for external animations. The 1.12.1 binary may not support external `.anim` files at all (1.12.1 may be inlined). The M2 options list at `0x0082e5xx–0x0082e7xx` does not include any animation cvar, suggesting 1.12.1 has no multithreaded animation toggle and likely no external animation seam.
