...
- **OQ-3**: The 1.12.1 vertex table uses 12-byte entries (0xc/entry) instead of 3.3.5's interleaved 0x30-0x40. **Status (spec 048)**: partially resolved (2026-06-06). The 048 reader now parses positions (3 floats, 0x0C stride), normals (3 floats, 0x0C stride), and vertex indices (u16 pair: posIdx, normIdx, 4 bytes/entry) from header offsets 0xEC, 0xF4, 0xFC respectively. UVs (2 floats, 0x08 stride) and triangles (u16, 0x02 stride) are also parsed from 0x104 and 0x10C. The new geometry is exposed as `M2ModelDocument.InlineEra1121Geometry` (3.3.5 readers leave it null). **Validation**: the 1.12.1 bear.m2 (363824 bytes) has positions 8 (forming a 2x2x2 cube), normals 12 (the 6 face normals of a cube), UVs 9, vertex indices 36 (the first ~20 reference valid positions/normals, the rest overflow into the position data region — likely the count=36 in the bear is a combined vertex + extension table), triangles 35 (suspicious — not a multiple of 3; possibly degenerate-triangle sentinel scheme). Batches 17 (suspicious — too many for a simple creature model). **OQ-TRIANGLE and OQ-BATCH remain deferred** to a follow-up slice. Phase 13 (wiring into the viewer) is also deferred.
