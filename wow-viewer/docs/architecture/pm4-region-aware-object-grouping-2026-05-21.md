...
The missing root layer is **MSHD.Field04 (Region ID)**. Empirical evidence from 616 development PM4 files shows:
...
**Field04 groups all objects belonging to a single level designer's work area.** It is the scene-division anchor.
...
### Level 0: Region (MSHD.Field04)

- Groups all tiles belonging to the same scene division / level designer area.
- A single region can span 2-13+ tiles in the development corpus.
- Region ID is per-tile (stored in MSHD header), not per-surface.
- Tiles with the same Field04 value form a region even if non-adjacent (same scene type).

### Level 1: Object (CK24)

- Within a region, CK24 groups surfaces into distinct collision objects.
...
2. Group tiles by MSHD.Field04 → region.
3. Within each region, group surfaces by CK24 → object.
...
7. Cross-tile merge: objects with the same CK24 across tiles in the same region are merged.
...
2. Group by Field04 → verify region count matches expected (227 distinct values).
3. Within each region, verify CK24 objects are correctly partitioned.
4. For the darkportal WMO (tile 3,1 region), verify its surfaces merge across 4 tiles under one region.
5. Verify MPRL position collection produces non-empty results for each sub-object.

## Validation Criteria

- Region count matches Field04 distinct value count (227 for development corpus).
- Each region's tile set matches the spatial clustering observed in cross-tile analysis.
- Darkportal WMO (tile 3,1) surfaces merge into one object across 4 tiles under one region.
...
- Cross-tile objects within the same region share the same merged CK24 key.
