...
| `MSLK` | `_0x00` type flags | **MEASURED** | **a bitfield, and bit 0 means the record carries NO geometry**. Values 0x01 and 0x11 carry a wall window 0.0% of the time across 443,882 records; the other eight values carry one 100.0% of the time across 386,351. No exceptions either way. Only 10 of 32 combinations occur, using bits 0-4, and **bits 1 and 2 never co-occur** - a two-state selector with bits 3 and 4 as independent modifiers |
| `MSLK` | `_0x04` | PARTIAL | **a group key whose groups are tiny** - 283,066 groups of exactly two against 250,192 singletons over 830,233 records, which is where the "near-unique" reading came from: 250,192 + 283,066 gives a distinct ratio of ~0.65 against the 0.676 measured. Values are always below the record count; they equal their own array index 0.21% of the time. **Not** an edge id joining half-edges - paired records are adjacent 7.11% of the time, share a `RefIndex` 0.38% (control 0.01%), and point at each other **0.08%**. Paired records share `TypeFlags` **99.82%** of the time. A pair is never a geometry record bound to an anchor: over 283,066 pairs, 77.09% are two anchors, 22.91% are two wall records, and one-of-each is **0.00%** - which follows from the shared `TypeFlags` once bit 0 is known to decide it. What a pair denotes is open |
...
| `MSLK` | `_0x01` subtype | **MEASURED as NOT a taxonomy** | 19 values that do not separate on anything: carries-wall 43-52%, mean window length 4.04, mean Z extent ~1.9 for every one of them, and each co-occurs with all 10 `_0x00` values. The counts decay like a counter rather than partitioning like a category - 229468, 218771, 211486, 131560, 28663, 7336, 1960, 613, 190, 84, 29, 26, 18, 8, 7, 6, 5, 2, 1. The name asserts a taxonomy the data denies; a sequence number or depth is the leading reading |
...
| `MPRL` | `Unk14`, `Unk16` | **MEASURED as one flag plus a counter** | `Unk16` takes two values, `0x0000` (101,851) and `0x3FFF` (39,367); `Unk14`'s `0xFFFF` bucket is **also 39,367**, so the two fields mark the same records - one sentinel carried redundantly. Excluding it, `Unk14` is a small counter decaying from zero (31950, 29916, 24357, 12712, 2016, 653, 165, 47, 12, ...), not 14 categories |
| `MPRL` | `Unk04` | **MEASURED** | **a footprint group id.** Points sharing a value sit a median **1.62** world units apart against **170.99** for random groups of the same size in the same file - roughly 105x tighter than chance, over 141,218 records. About 166 groups per file. It says which footprint a terrain-contact point belongs to. **It is NOT an angle:** the observed range is 0..38317 and a 16-bit heading scaled by `2*pi/65536` would reach ~65535. This project had such a conversion live in `Pm4ObjectPositionDecoder`; it has been removed |
| `MPRL` | `Unk00` | PARTIAL | averages **1.0 distinct value per file**, so it is a per-TILE value rather than a per-point one; 95 distinct across 200 files. What it identifies is open. Note a spatial-coherence test on it is vacuous - with one group per file, within-group distance equals its own control by construction |
