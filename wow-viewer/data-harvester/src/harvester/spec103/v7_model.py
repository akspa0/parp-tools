"""MultiChannelUNetV7, ported from gillijimproject_refactor (read-only reference).

Architecture, channel widths, heads, and the WDL-trestle path are unchanged from
`gillijimproject_refactor/src/WoWMapConverter/scripts/v7_model.py`. The only deviation,
recorded in specs/103-image-only-reconstruction/research-v7-contract.md §5: the hardcoded
module constant OUTPUT_SIZE=512 became the constructor argument `output_size` (default 256,
our native minimap resolution).
"""
