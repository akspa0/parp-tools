"""Canonical v50 owner for writing visible OBJ/PNG review artifacts for a WDL evaluation.

Depends on the standalone ``height_to_obj`` mesh helper in ``scripts/spec103_export_mesh.py``,
which is a shared utility (not itself a v50-owned command) and is reached via the scripts
directory on ``sys.path``, same as every other script-to-script import in this codebase.
"""
...
def main() -> int:
    ...
    ap.add_argument("--evaluation", required=True, type=Path, help="v50_review_wdl_prior.py output directory")
