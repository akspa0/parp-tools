...
def build_training_curriculum(
    *,
    ...
    """Write the curriculum store and return its summary dict."""
    import zarr
            ...
            selected.append(
                {
                    ...
                    "split": "val" if str(row["map"]) == val_map else "train",
...
def main() -> int:
    ...
    ap.add_argument("--val-map", required=True, help="whole map held out as validation (e.g. PVPZone02)")
    ...
    print(
        ...
        f"train={summary['splits']['train']} val={summary['splits']['val']} (val_map={summary['val_map']}) "
        f"-> {args.output}"
    )
    return 0
