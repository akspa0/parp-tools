...
def materialize_coarse_relief(
    *,
    ...
    try:
        model, checkpoint, _identity = load_geometry_checkpoint(checkpoint_path, device=device)
    ...
    for position, row_index in enumerate(selected):
        ...
        array[position] = predict_relief(model, rgb, device=device).astype(np.float16)
