"""Spec 117 T016: the bridge writes exactly the shape/schema/attrs the existing (unmodified)
geometry trainers already validate for ``--feature-store``, never mutates the source store, and
refuses to clobber an existing output directory."""
...
from harvester.spec117.lattice_bridge import (
    CLASS_COUNT,
    FEATURE_ARRAY,
...
from harvester.spec117.lattice_contract import architecture_identity
...
def _build_checkpoint(tmp_path: Path, *, base: int = 8) -> Path:
    ...
    identity = architecture_identity(model, architecture_id="lattice_net", config={"base": base})
    checkpoint = {
        "architecture": identity,
        "lattice_config": {"base": base},
...
def test_dry_run_returns_plan_and_writes_nothing(tmp_path: Path):
    ...
    plan = lattice_to_feature_map(store=store, checkpoint=checkpoint, output=output, write=False)
    assert plan["schema"] == "v117-lattice-bridge-plan-v1"
...
def test_write_produces_the_exact_contract_the_trainers_already_validate(tmp_path: Path):
    ...
    checkpoint = _build_checkpoint(tmp_path)
    output = tmp_path / "feature_store"
    original_minimap_bytes = (store / "index.parquet").read_bytes()

    result = lattice_to_feature_map(store=store, checkpoint=checkpoint, output=output, write=True)
    ...
    assert dict(group.attrs)["source_signal"] == "wdl_lattice"
    ...
    assert (store / "index.parquet").read_bytes() == original_minimap_bytes
...
def test_refuses_to_overwrite_a_nonempty_output(tmp_path: Path):
    ...
    checkpoint = _build_checkpoint(tmp_path)
    output = tmp_path / "feature_store"
    lattice_to_feature_map(store=store, checkpoint=checkpoint, output=output, write=True)

    with pytest.raises(LatticeBridgeError, match="non-empty"):
        lattice_to_feature_map(store=store, checkpoint=checkpoint, output=output, write=True)
...
def test_refuses_a_store_without_minimap_rgb(tmp_path: Path):
    ...
    with pytest.raises(LatticeBridgeError, match="minimap_rgb"):
        lattice_to_feature_map(store=store, checkpoint=checkpoint, output=tmp_path / "out", write=False)
