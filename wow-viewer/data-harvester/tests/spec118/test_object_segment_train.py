...
def test_required_arrays_match_the_us1_catalog_names():
    assert set(REQUIRED_ARRAYS) == {"minimap_rgb", "object_mask"}
...
def test_require_object_arrays_refuses_a_store_missing_the_source_array():
    with pytest.raises(TrainerContractError, match="object_mask"):
...
def test_build_object_plan_records_the_recipe_and_gate_thresholds():
    ...
    assert plan["training_target"] == "object_mask (v18 placement footprint) -> object_class_2 (none/object)"
