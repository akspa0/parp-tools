...
def _pick_tiles(group, count: int) -> list[int]:
    """The `count` busiest occupied tiles (most doodads, real relief) — best for eyeballing."""
    import numpy as np

    n = group["height_257"].shape[0]
    counts = np.asarray(group["mddf_count"]) if "mddf_count" in group else np.zeros(n)
    occupied = [i for i in range(n) if float(np.asarray(group["height_257"][i]).std()) >= 2.0]
    occupied.sort(key=lambda i: int(counts[i]), reverse=True)
    return occupied[:count] if occupied else [0]
...
def _draw(ax, name: str, arr: np.ndarray) -> str:
    ...
    if a.ndim == 2:
        ...
        return f"[{lo:.2f},{hi:.2f}]" if hi > lo else f"empty (all {lo:.2f})"
...
def main() -> int:
    ...
    ap.add_argument("--count", type=int, default=1,
                    help="render this many of the busiest occupied tiles (one sheet each); ignored "
                         "when a specific tile is given")
    args = ap.parse_args()
    ...
    if args.tile_x is not None and args.tile_y is not None:
        ...
        tiles = [args.tile_index]
    else:
        tiles = _pick_tiles(group, max(1, args.count))
    ...
    for tile in tiles:
        ...
        out = args.output if (args.output and len(tiles) == 1) else (
            args.store.parent / f"validation-sheet-{meta.get('map','tile')}-{meta.get('tile_x','x')}-{meta.get('tile_y','y')}.png")
        fig.tight_layout(rect=(0, 0, 1, 0.985))
        fig.savefig(out, dpi=110)
        plt.close(fig)
        print(f"wrote {out}  ({tile_label} | {len(names)} panels | doodads={n_doodads} wmos={n_wmos})")
    return 0
