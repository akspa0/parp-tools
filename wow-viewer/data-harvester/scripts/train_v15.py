"""Train V15 terrain model — minimap → terrain mesh.

Usage:
    cd wow-viewer/data-harvester
    uv run python scripts/train_v15.py
    uv run python scripts/train_v15.py --run-name my_experiment
    uv run python scripts/train_v15.py --resume-checkpoint wow-viewer/models/v15/runs/<run>/checkpoints/v15_best.pt
"""
...
from harvester.v15_dataset import V15Dataset  # noqa: E402
from harvester.v15_model import V15Model  # noqa: E402

_PROJECT_ROOT = Path(__file__).resolve().parent.parent.parent
MODELS_ROOT = _PROJECT_ROOT / "models"
DATASET_ROOT = _PROJECT_ROOT / "output" / "datasets"

SHARD_DIRS = [
    DATASET_ROOT / "d1_reharvest" / "shards",
]
DEFAULT_SHARD_ROOT = SHARD_DIRS[0]
DEFAULT_VALIDATION_JSON = (
    DATASET_ROOT / "full_shard_batch_staged_native" / "manifests" / "validation_selection.json"
)


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Train V15 terrain model")
    ...
    p.add_argument("--val-snapshots", type=int, default=5,
                   help="Number of validation tiles to export as images per validation run")
...
def main() -> None:
    ...
    run_dir = MODELS_ROOT / "v15" / "runs" / run_name
    ...
    train_ds = V15Dataset(args.shard_root, args.validation_json, split="train",
                          max_samples=args.max_samples or None, seed=args.seed,
                          augment=args.augment)
    val_ds = V15Dataset(args.shard_root, args.validation_json, split="val",
