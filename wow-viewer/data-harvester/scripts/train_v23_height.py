...
def _save_val_preview(batch: dict[str, Any], outputs: Any, preview_dir: Path, epoch: int) -> None:
    ...
    for idx in range(count):
        ...
        Image.fromarray(strip, mode="RGB").save(preview_dir / f"tile_{idx}.png")
...
def _run_loader(
    *,
    ...
    del amp_label
    return averaged.get("total", 0.0), averaged, preview_payload
...
def _train_once(args: argparse.Namespace) -> dict[str, Any]:
    ...
    sample_channels = next(iter(train_loader))["input"].shape[1]
    ...
    for epoch in range(start_epoch, int(args.epochs) + 1):
        ...
        save_checkpoint(ckpt_dir / "v23_height_last.pt", checkpoint)
        if val_loss <= best_val:
            best_val = val_loss
            save_checkpoint(ckpt_dir / "v23_height_best.pt", checkpoint)
    ...
    return {"run_dir": run_dir, "best_val": best_val, "history": history}
