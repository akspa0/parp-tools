...
def main() -> int:
    ...
    real_candidates = []
    rejected_dark = rejected_object = 0
    for row_number, row in enumerate(_records(args.real_store)):
        if not (row.get("has_alpha_256") and row.get("has_height_257") and row.get("has_minimap_rgb")):
            continue
        rgb_mean = float(np.asarray(real_group["minimap_rgb"][row_number], dtype=np.float32).mean())
        if rgb_mean < args.min_rgb_mean:
            rejected_dark += 1
            continue
        if "object_precise_mask" in real_group:
            object_coverage = float((np.asarray(real_group["object_precise_mask"][row_number]) > 0.5).mean())
            if object_coverage > args.max_object_coverage:
                rejected_object += 1
                continue
    ...
    selected_real = select_real_rows(real_candidates, total=args.real_rows)
    # Synthetic lighting variants are source groups of three. Keep whole groups and
    # reduce real selection only enough to retain the exact cap without reintroducing
    # a rejected real row.
    selected_real = selected_real[: len(selected_real) - ((args.max_rows - len(selected_real)) % 3)]
    selected_synthetic = select_synthetic_rows(synthetic_candidates, total=args.max_rows - len(selected_real))
    selected = selected_real + selected_synthetic
