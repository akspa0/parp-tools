...
- Repeatable GitHub-facing setup for the current direct training flow now lives in `wow-viewer/docs/validation/direct-v9-training-setup.md`, including staged WoWArchive builds, `3_0_1_8303/Northrend`, and the current development-map dev-eval cache path.
...
- The new direct `v9` wrapper defaults `IncludeBuilds` to `0_5_3_3368`, `0_5_5_3494`, `0_7_0_3694`, `3_0_1_8303`, `3_3_5_12340`, and `4_0_0_11927`; the early-build family is expected to come from staged copies under `output/tmp/wowarchive-clients`, not direct archive streaming.
- `3_0_1_8303` is now included specifically so the direct dataset path can pull `Northrend`, which is the current closest large-scale source surface for terrain and content patterns later copied into the development map.
- `4_0_0_11927` should also be part of the main corpus for the same reason: it is from the same era as the development map and still carries the map-family data needed for reconstruction.
- The current recommended held-out validation surface is still `datasets/original_development/development`, with `development_0_0` treated as the first trusted pinned tile and the broader root usable as a separate `train_v9.py --dev-eval-cache-manifest` holdout.
- That held-out development root already includes loose-overlay-derived object-marking surfaces on object-bearing tiles, including `object_visibility_mask`, `pm4_mask`, `no_object_minimap`, and `terrain_only_minimap`; do not describe it as minimap-only or terrain-only validation data.
- Archive-only bounded proof now exists for staged `0_7_0_3694`: `dataset-scan`, `dataset-merge`, `dataset-audit`, `dataset-curate`, `dataset-build-cache`, and downstream trainer audit all completed from the staged WoWArchive copy.
- Archive-only bounded proof now also exists for staged `0_5_3_3368/Azeroth` through the direct embedded-Alpha path: `dataset-scan`, `dataset-audit`, and `dataset-build-cache` all completed from the staged WoWArchive copy using `Azeroth.wdt.MPQ#alpha-tile(0,0)` reopen markers.
- Archive-only bounded wrapper proof now also exists across staged `0_5_3_3368`, `0_5_5_3494`, and `0_7_0_3694`: `run_v9_direct_pipeline.ps1 -Mode audit -WowArchiveOnly -NoRequireMinimap -NoRequireWdl` completed end to end with `MergedSamples = 7`, `Curated = 4`, `Processed = 4`, and trainer audit `rejected = 0`.
- A later bounded staged-wrapper rerun with the default gates enabled completed end to end after two narrow repairs: `0_7_0_3694/Azeroth` no longer hits the old direct-scan parse failure, and `0_7_0_3694:EmeraldDream_24_25` now resolves a real archive-backed minimap through `textures\minimap\875b7ac8e14d5601824624a357b340c1.blp`.
- The remaining default-gate rejection on that staged smoke set is now `1/4` rather than `2/4`: `0_5_5_3494:EmeraldDream_24_25` still reports `missing_minimap_rgb_256`, so minimap availability remains the first real default-gate blocker rather than WDL.
- Follow-up side-by-side checks show that the staged `0_5_5_3494` EmeraldDream miss is not just a direct-path port bug: the current legacy `WoWMapConverter` exporter also reports `Minimap not found for EmeraldDream 24_25` on the same staged client root.
- The stronger build-content evidence is: staged `0_5_5_3494` exposes no `map24_25.blp` entry anywhere in root `Data\textures\Minimap\md5translate.txt`; local `0.6.0` test data does contain an `Emerald Dream` block but still not `map24_25.blp`; staged `0_7_0_3694` is the first checked build here that exposes `Emerald Dream\map24_25.blp`.
- Keep the remaining boundary explicit: that bounded default-gate proof is still not broad early-build closure, and the surviving blocker now looks like a real early-build EmeraldDream minimap-content/translation boundary rather than the old `0_7_0_3694/Azeroth` scan failure.
