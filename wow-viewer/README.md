...
- `tests/WowViewer.Core.Tests` now locks the current FourCC and chunk-header boundary behavior.
	...
	- it now also locks synthetic and real standard-archive `MDX` top-level-plus-`SEQS`-and-`TEXS`-and-`MTLS` summary behavior for `world/generic/activedoodads/chest01/chest01.mdx`, plus a real animated-asset probe path for `world/generic/passivedoodads/particleemitters/greengroundfog.mdx`
...
- This is intentionally narrow for now:
	...
	- it now also reports a first shared `MDX` top-level summary for `MDLX` signature, chunk order, known-vs-unknown chunk coverage, `VERS`, narrow `MODL` name or bounds or blend-time signals, shared `SEQS` sequence signals, shared `TEXS` texture-table paths or flags, and narrow `MTLS` material-layer signals when a model file is inspected
