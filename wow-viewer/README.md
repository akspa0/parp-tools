...
- **Modern CASC Clients (alpha)**: File → Open CASC Install (local, or local + CDN fill), then pick the game version. Tested with WoW: Forever (`wow_classic_beta` 1.60.1.69876). Covers FileDataID-era maps (`MAID`, `MDID`, FileDataID placements), chunked `MD21` M2s, `GFID`/`MODI` WMOs with modern per-LOD groups and materials, and DB2 tables. Terrain blends up to 8 tileset layers per chunk at full resolution. See [release notes](docs/releases/v0.6.0-alpha.md).
- **DAT v26 Terrain (alpha)**: File → Open DAT v26 Terrain Folder renders the raw terrain project files that shipped in that build — heights (inches), normals, vertex colours, layers and `ACDO` objects — and File → Export Nearby Tiles as DAT v26 (experimental) writes them back ([format write-up](docs/architecture/adt-v26-format.md)).
...
| **WoW: Forever** | 1.60.1.69876 (`wow_classic_beta`, 12.0-based, CASC) | **Alpha (v0.6.0-alpha)** | Local CASC + optional CDN fill + game-version picker; `MAID`/`MDID` terrain with up to 8 blended layers, FileDataID placements, `MD21` M2, `GFID`/`MODI` WMO with modern materials/LOD, DB2 via WoWDBDefs; DAT v26 terrain project files (reader, renderer, experimental writer) |
