...
internal sealed class Pm4OverlayCacheService
{
    ...
    // Spec 054: bumped to 8 to invalidate the per-window cache when the
    ...
    /// <summary>
    /// The root directory for this cache service. Spec 054: the
    /// per-file PM4 overlay cache (managed by
    /// <c>Pm4PerFileCacheService</c>) lives in a <c>files/</c> subdir
    /// under this root for the same data source, so the two layers
    ...
    internal static bool TryGetLooseFileStamp(IDataSource dataSource, string virtualPath, out long fileLength, out long lastWriteTicks)
    {
        fileLength = 0;
        lastWriteTicks = 0;
