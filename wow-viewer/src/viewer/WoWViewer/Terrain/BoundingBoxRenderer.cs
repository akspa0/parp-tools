...
void main() { FragColor = vColor; }";
    ...
    /// <summary>
    /// Add a wireframe octahedron marker at a center point with given half-extent.
    /// 6 vertices, 12 edges — reads as a clear vertex pinpoint, never fills the volume.
    /// </summary>
    public void BatchOctahedron(Vector3 center, float halfExtent, Vector3 color)
    {
        // 6 vertices: +X, -X, +Y, -Y, +Z, -Z
        Vector3 pX = center + new Vector3(halfExtent, 0, 0);
        Vector3 nX = center + new Vector3(-halfExtent, 0, 0);
        Vector3 pY = center + new Vector3(0, halfExtent, 0);
        Vector3 nY = center + new Vector3(0, -halfExtent, 0);
        Vector3 pZ = center + new Vector3(0, 0, halfExtent);
        Vector3 nZ = center + new Vector3(0, 0, -halfExtent);

        // 12 edges — each axis-tip connects to the 4 tips on the other two axes
        BatchLine(pX, pY, color); BatchLine(pX, nY, color);
        BatchLine(pX, pZ, color); BatchLine(pX, nZ, color);
        BatchLine(nX, pY, color); BatchLine(nX, nY, color);
        BatchLine(nX, pZ, color); BatchLine(nX, nZ, color);
        BatchLine(pY, pZ, color); BatchLine(pY, nZ, color);
        BatchLine(nY, pZ, color); BatchLine(nY, nZ, color);
    }
    ...
    /// <summary>
    /// Add a solid axis-aligned box (12 triangles, 36 vertices) to the solid batch.
    /// Use with FlushSolidBatch — caller configures blend/depth state.
    /// </summary>
    public void BatchSolidBoxMinMax(Vector3 min, Vector3 max, Vector3 color, float alpha)
    {
        // 8 unique corners
        ...
        // Bottom face (v0,v1,v2,v3) - looking from -Z
        BatchTriangle(v0, v1, v2, color, alpha);
        BatchTriangle(v0, v2, v3, color, alpha);
        // Top face (v4,v5,v6,v7) - looking from +Z
        BatchTriangle(v4, v6, v5, color, alpha);
        BatchTriangle(v4, v7, v6, color, alpha);
        // Front face (v0,v1,v5,v4) - looking from -Y
        BatchTriangle(v0, v5, v1, color, alpha);
        BatchTriangle(v0, v4, v5, color, alpha);
        // Back face (v3,v2,v6,v7) - looking from +Y
        BatchTriangle(v3, v2, v6, color, alpha);
        BatchTriangle(v3, v6, v7, color, alpha);
        // Left face (v0,v3,v7,v4) - looking from -X
        BatchTriangle(v0, v3, v7, color, alpha);
        BatchTriangle(v0, v7, v4, color, alpha);
        // Right face (v1,v2,v6,v5) - looking from +X
        BatchTriangle(v1, v6, v2, color, alpha);
        BatchTriangle(v1, v5, v6, color, alpha);
    }

    /// <summary>
    /// Add a solid colored cube at a center point with given half-extent.
    /// Convenience wrapper around BatchSolidBoxMinMax.
    /// </summary>
    public void BatchSolidCube(Vector3 center, float halfExtent, Vector3 color, float alpha)
    {
        Vector3 size = new(halfExtent);
        BatchSolidBoxMinMax(center - size, center + size, color, alpha);
    }
