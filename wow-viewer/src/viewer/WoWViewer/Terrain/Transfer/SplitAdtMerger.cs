...
public sealed class SplitAdtMerger
{
    ...
    public MergeResult Merge(string rootPath, string? obj0Path = null, string? tex0Path = null)
    {
        ...
        try
        {
            ...
            Warcraft.NET.Files.ADT.Terrain.Wotlk.Terrain wotlkAdt;
            ...
            else
            {
                ...
                try
                {
                    wotlkAdt = new Warcraft.NET.Files.ADT.Terrain.Wotlk.Terrain(rootBytes);
                }
                catch (Exception ex)
                {
    ...
    /// </summary>
    private Warcraft.NET.Files.ADT.Terrain.Wotlk.Terrain BuildWotlkFromCataSplit(byte[] rootBytes, string? obj0Path, string? tex0Path, MergeResult result)
    {
        var wotlkAdt = new Warcraft.NET.Files.ADT.Terrain.Wotlk.Terrain();
        ...
        // Parse MCNKs from root (Warcraft.NET.Files.ADT.Terrain.Wotlk.Terrain data)
    ...
    /// </summary>
    private void MergeSplitFiles(Warcraft.NET.Files.ADT.Terrain.Wotlk.Terrain wotlkAdt, string? obj0Path, string? tex0Path, MergeResult result)
    {
    ...
    /// </summary>
    private void MergeTextureData(Warcraft.NET.Files.ADT.Terrain.Wotlk.Terrain root, Warcraft.NET.Files.ADT.TerrainTexture.Legion.TerrainTexture tex0)
    {
    ...
    /// </summary>
    private void MergeObjectData(Warcraft.NET.Files.ADT.Terrain.Wotlk.Terrain root, Warcraft.NET.Files.ADT.TerrainObject.Zero.TerrainObjectZero obj0)
    {
        // Merge M2 model data
    ...
    public void CompareWithReference(string mergedPath, string referencePath)
    {
        ...
        try
        {
            var merged = new Warcraft.NET.Files.ADT.Terrain.Wotlk.Terrain(mergedBytes);
            var reference = new Warcraft.NET.Files.ADT.Terrain.Wotlk.Terrain(refBytes);
