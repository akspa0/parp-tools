...
public class MinimapBakeService
{
    ...
    /// </summary>
    public async Task<SixLabors.ImageSharp.Image<Rgba32>> BakeTileAsync(string jsonPath)
    {
        ...
        var fullTile = new SixLabors.ImageSharp.Image<Rgba32>(16 * 256, 16 * 256);
        ...
        foreach (var chunk in sample.TerrainData.ChunkLayers)
        {
            ...
            fullTile.Mutate(ctx => ctx.DrawImage(chunkImage, new SixLabors.ImageSharp.Point(col * 256, row * 256), 1.0f));
        }
    ...
    public async Task<(SixLabors.ImageSharp.Image<Rgba32> Composite, int LayerCount, string Stats)> BakeTileWithLayersAsync(
        string jsonPath, string outputDir, bool applyShadows = true)
    {
        ...
        var rawLayers = new SixLabors.ImageSharp.Image<Rgba32>[maxLayers];
        var weightedLayers = new SixLabors.ImageSharp.Image<Rgba32>[maxLayers];
        var cumulativeLayers = new SixLabors.ImageSharp.Image<Rgba32>[maxLayers];
        var shadowedLayers = new SixLabors.ImageSharp.Image<Rgba32>[maxLayers];
        
        for (int i = 0; i < maxLayers; i++)
        {
            rawLayers[i] = new SixLabors.ImageSharp.Image<Rgba32>(16 * 256, 16 * 256);
            weightedLayers[i] = new SixLabors.ImageSharp.Image<Rgba32>(16 * 256, 16 * 256);
            cumulativeLayers[i] = new SixLabors.ImageSharp.Image<Rgba32>(16 * 256, 16 * 256);
            shadowedLayers[i] = new SixLabors.ImageSharp.Image<Rgba32>(16 * 256, 16 * 256);
        }

        // Full tile shadow mask (composite of all chunk shadows)
        var fullShadowMask = new SixLabors.ImageSharp.Image<L8>(16 * 256, 16 * 256);
        ...
        var composite = new SixLabors.ImageSharp.Image<Rgba32>(16 * 256, 16 * 256);
        var compositeWithShadows = new SixLabors.ImageSharp.Image<Rgba32>(16 * 256, 16 * 256);
        ...
        foreach (var chunk in sample.TerrainData.ChunkLayers)
        {
            var row = chunk.ChunkIndex / 16;
            var col = chunk.ChunkIndex % 16;
            var point = new SixLabors.ImageSharp.Point(col * 256, row * 256);
            
            // Load shadow mask for this chunk if available
            SixLabors.ImageSharp.Image<L8>? chunkShadow = null;
            if (applyShadows && !string.IsNullOrEmpty(chunk.ShadowPath))
            {
                var shadowPath = Path.Combine(_datasetRoot, chunk.ShadowPath);
                if (File.Exists(shadowPath))
                {
                    chunkShadow = await SixLabors.ImageSharp.Image.LoadAsync<L8>(shadowPath);
    ...
    /// </summary>
    private async Task<(SixLabors.ImageSharp.Image<Rgba32>? Raw, SixLabors.ImageSharp.Image<Rgba32>? Weighted, SixLabors.ImageSharp.Image<Rgba32>? Cumulative)[]> BakeChunkLayersAsync(VlmChunkLayers chunk)
    {
        const int size = ChunkBakeSize;
        
        if (chunk.Layers == null || chunk.Layers.Length == 0)
            return Array.Empty<(SixLabors.ImageSharp.Image<Rgba32>?, SixLabors.ImageSharp.Image<Rgba32>?, SixLabors.ImageSharp.Image<Rgba32>?)>();

        var results = new (SixLabors.ImageSharp.Image<Rgba32>?, SixLabors.ImageSharp.Image<Rgba32>?, SixLabors.ImageSharp.Image<Rgba32>?)[chunk.Layers.Length];
        
        // Load all textures and alpha masks first
        var textures = new SixLabors.ImageSharp.Image<Rgba32>?[chunk.Layers.Length];
        ...
        var cumulative = new SixLabors.ImageSharp.Image<Rgba32>(size, size);
        ...
        for (int layerIdx = 0; layerIdx < chunk.Layers.Length; layerIdx++)
        {
            ...
            var weightedTex = new SixLabors.ImageSharp.Image<Rgba32>(size, size);
    ...
    /// </summary>
    public Task<SixLabors.ImageSharp.Image<Rgba32>> BakeChunkAsync(VlmChunkLayers chunk)
    {
        return BakeChunkAsync(chunk, fallbackTexturePath: null);
    }

    public async Task<SixLabors.ImageSharp.Image<Rgba32>> BakeChunkAsync(VlmChunkLayers chunk, string? fallbackTexturePath)
    {
        return await TryBakeChunkAsync(chunk, fallbackTexturePath) ?? new SixLabors.ImageSharp.Image<Rgba32>(ChunkBakeSize, ChunkBakeSize);
    }

    public async Task<SixLabors.ImageSharp.Image<Rgba32>?> TryBakeChunkAsync(VlmChunkLayers chunk, string? fallbackTexturePath = null)
    {
        const int size = ChunkBakeSize;
        ...
        var chunkImage = new SixLabors.ImageSharp.Image<Rgba32>(size, size);
    ...
    private async Task<SixLabors.ImageSharp.Image<Rgba32>?> LoadChunkTextureAsync(string? texturePath, int size)
    {
        ...
        SixLabors.ImageSharp.Image<Rgba32> texture = await SixLabors.ImageSharp.Image.LoadAsync<Rgba32>(texPath);
    ...
    private async Task<byte[]?> LoadLayerAlphaAsync(VlmTextureLayer layer, int size)
    {
        ...
        using var maskImage = await SixLabors.ImageSharp.Image.LoadAsync<L8>(maskPath);
    ...
    private byte[]? ExpandAlphaBytes(byte[] alphaBytes, int size)
    {
        ...
        using var alphaImage = SixLabors.ImageSharp.Image.LoadPixelData<L8>(alphaBytes, sourceSize, sourceSize);
    ...
    private byte[] FlattenAlphaImage(SixLabors.ImageSharp.Image<L8> alphaImage)
    {
    ...
    private void ApplyAlphaMask(SixLabors.ImageSharp.Image<Rgba32> image, SixLabors.ImageSharp.Image<L8> mask)
    {
    ...
    public async Task<SixLabors.ImageSharp.Image<Rgba32>> BakeTileWithShadowsAsync(string jsonPath, bool applyShadows = true)
    {
        if (!File.Exists(jsonPath))
            throw new FileNotFoundException("JSON tile not found", jsonPath);
        ...
        foreach (var chunk in sample.TerrainData.ChunkLayers)
        {
            ...
            if (applyShadows && !string.IsNullOrEmpty(chunk.ShadowPath))
            {
                ...
                if (File.Exists(shadowPath))
                {
                    using var shadowImage = await SixLabors.ImageSharp.Image.LoadAsync<L8>(shadowPath);
    ...
    /// </summary>
    private void ApplyShadowOverlay(SixLabors.ImageSharp.Image<Rgba32> image, SixLabors.ImageSharp.Image<L8> shadow)
    {
    ...
    /// </summary>
    private void RemoveShadowOverlay(SixLabors.ImageSharp.Image<Rgba32> image, SixLabors.ImageSharp.Image<L8> shadow)
    {
    ...
    public async Task<SixLabors.ImageSharp.Image<Rgba32>> DebakeShadowsFromMinimapAsync(string minimapPath, string jsonPath)
    {
        ...
        using var sourceImage = await SixLabors.ImageSharp.Image.LoadAsync<Rgba32>(minimapPath);
    ...
    public async Task<SixLabors.ImageSharp.Image<Rgba32>> BakeShadowsOntoMinimapAsync(string cleanMinimapPath, string jsonPath)
    {
        ...
        using var sourceImage = await SixLabors.ImageSharp.Image.LoadAsync<Rgba32>(cleanMinimapPath);
        var result = sourceImage.Clone();
