...
public class VlmDatasetExporter
{
            ...
            foreach (var jsonPath in jsonFiles)
            {
                ...
                try
                {
                    ...
                    if (sample != null && sample.TerrainData != null)
                    {
                        ...
                        {
                            using SixLabors.ImageSharp.Image<Rgba32> minimapForMask = SixLabors.ImageSharp.Image.Load<Rgba32>(sourceMinimapPath);
                            if (minimapForMask.Width > 0 && minimapForMask.Height > 0)
                            {
                                ...
                                {
                                    IReadOnlyList<Pm4MprlEntry> positionRefs = LoadPm4PositionRefs(
                                        searchPaths,
                                        archiveCatalog,
                                        mapName,
                                        mapDirectory,
        ...
        uint wdtMphdFlags = 0)
    {
        ...
        try
        {
            ...
            Dictionary<int, WowViewer.Core.IO.Lk.Mcnk>? texMcnkByChunkIndex = null;
            if (texBytes is { Length: > 16 })
            {
                ...
                texMcnkByChunkIndex = new Dictionary<int, WowViewer.Core.IO.Lk.Mcnk>();
                ...
                var tex0ParseOptions = new WowViewer.Core.IO.Lk.Mcnk.ParseOptions { SkipHeader = true };
                for (int ci = 0; ci < Math.Min(256, texOffsets.Count); ci++)
                {
                    ...
                    try
                    {
                        var texMcnk = new WowViewer.Core.IO.Lk.Mcnk(mcnkBody, tex0ParseOptions);
            ...
            for (int chunkIndex = 0; chunkIndex < 256 && chunkIndex < mcnkOffsets.Count; chunkIndex++)
            {
                ...
                try 
                {
                    var mcnk = new WowViewer.Core.IO.Lk.Mcnk(mcnkBody);
                    ...
                    // 4. Shadows (MCSH) — prefer _tex0 MCNK shadow if base has none (matches WoWViewer)
    ...
    private async Task<string?> GenerateHeightmap(List<VlmChunkHeights> chunkHeights, string tileName, string outputDir, bool isInterleaved)
    {
        ...
        using (SixLabors.ImageSharp.Image<L16> image = TerrainTileBakeService.CreateHeightmapImage(tileHeightmap.Heights, tileHeightmap.MinHeight, tileHeightmap.MaxHeight, 256))
        {
    ...
    private async Task<string?> GenerateNormalmap(List<VlmChunkHeights> chunkHeights, int[] holes, string tileName, string outputDir, bool isInterleaved)
    {
        ...
        Directory.CreateDirectory(imagesDir);
        var path = Path.Combine(imagesDir, filename);
        using (SixLabors.ImageSharp.Image<Rgba32> image = TerrainTileBakeService.CreateNormalmapImage(tileNormals, 256))
        {
            await image.SaveAsPngAsync(path);
        }
    ...
    private byte[] BuildObjectVisibilityMask(VlmTerrainData terrainData, int width, int height, IArchiveReader archiveReader, IReadOnlyList<string> searchPaths)
    {
        ...
        using SixLabors.ImageSharp.Image<L8> objectMask = new(width, height);
    ...
    private static IReadOnlyList<Pm4MprlEntry> LoadPm4PositionRefs(
        IReadOnlyList<string> searchPaths,
        ...
        try
        {
            foreach (string candidate in BuildPm4InternalPathCandidates(mapName, mapDirectory, tileX, tileY))
            {
                foreach (string basePath in searchPaths)
                {
                    ...
                    Pm4ResearchDocument diskPm4File = Pm4ResearchReader.ReadFile(diskCandidate);
                    return diskPm4File.KnownChunks.Mprl;
                }
                ...
                Pm4ResearchDocument archivePm4File = Pm4ResearchReader.Read(data);
                return archivePm4File.KnownChunks.Mprl;
            }
            ...
            foreach (string fileName in BuildPm4FileNameCandidates(mapName, mapDirectory, tileName, tileX, tileY))
            {
                foreach (string basePath in searchPaths)
                {
                    ...
                    Pm4ResearchDocument loosePm4File = Pm4ResearchReader.ReadFile(diskCandidate);
                    return loosePm4File.KnownChunks.Mprl;
                }
            }
        }
        catch
    ...
    private static bool FillMaskPolygon(SixLabors.ImageSharp.Image<L8> image, IReadOnlyList<Vector2> polygon)
    {
        if (polygon.Count < 3)
            return false;
    ...
    private static void DrawFilledEllipse(SixLabors.ImageSharp.Image<L8> image, int centerX, int centerY, int radiusX, int radiusY)
    {
    ...
    private static byte[] SynthesizeMaskedMinimap(string sourceMinimapPath, byte[] maskPngBytes)
    {
        try
        {
            using var srcImage = SixLabors.ImageSharp.Image.Load<Rgba32>(sourceMinimapPath);
            int w = srcImage.Width;
            int h = srcImage.Height;

            using var maskStream = new MemoryStream(maskPngBytes);
            using var maskImage = SixLabors.ImageSharp.Image.Load<L8>(maskStream);
            ...
            using var result = new SixLabors.ImageSharp.Image<Rgba32>(w, h);
        ...
        MinimapBakeService minimapBakeService)
    {
        ...
        try
        {
            using var result = SixLabors.ImageSharp.Image.Load<Rgba32>(sourceMinimapPath);
            using var maskStream = new MemoryStream(maskPngBytes);
            using var maskImage = SixLabors.ImageSharp.Image.Load<L8>(maskStream);
    ...
    internal static async Task<bool> ReplaceMaskedPixelsWithBakedChunksAsync(
        ...
        SixLabors.ImageSharp.Image<L8> maskImage,
        VlmChunkLayers[]? chunkLayers,
        Func<VlmChunkLayers, string?, Task<SixLabors.ImageSharp.Image<Rgba32>?>> bakeChunkAsync)
    {
        ...
        foreach (VlmChunkLayers chunk in chunkLayers)
        {
            ...
            SixLabors.ImageSharp.Image<Rgba32>? bakedChunk = await bakeChunkAsync(chunk, fallbackTexturePath);
            if (bakedChunk == null)
                continue;
    ...
    private static byte[] CombineMaskPngBytes(params byte[][] maskPngs)
    {
        ...
        try
        {
            ...
            using SixLabors.ImageSharp.Image<L8> merged = SixLabors.ImageSharp.Image.Load<L8>(firstStream);

            for (int index = 1; index < validMasks.Length; index++)
            {
                using MemoryStream overlayStream = new(validMasks[index]);
                using SixLabors.ImageSharp.Image<L8> overlay = SixLabors.ImageSharp.Image.Load<L8>(overlayStream);
    ...
    internal static byte[] RenderChunkValueMap(ushort[] chunkValues, int width, int height)
    {
        ...
        using var image = new SixLabors.ImageSharp.Image<L16>(width, height);
    ...
    private static bool ChunkHasMaskedPixels(SixLabors.ImageSharp.Image<L8> maskImage, (int StartX, int StartY, int EndX, int EndY) bounds)
    {
        for (int y = bounds.StartY; y < bounds.EndY; y++)
        {
            for (int x = bounds.StartX; x < bounds.EndX; x++)
    ...
    private async Task GenerateGlobalHeightmapsAsync(string datasetDir, string outputDir, IProgress<string>? progress)
    {
        ...
        foreach (var jsonPath in jsonFiles)
        {
            try
            {
                ...
                using (SixLabors.ImageSharp.Image<L16> image = TerrainTileBakeService.CreateHeightmapImage(tileHeightmap.Heights, globalMin, globalMax, 512))
                {
                    await image.SaveAsPngAsync(path);
                }
