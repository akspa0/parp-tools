...
void main() {
    ...
    private void DrawMlFinalizeSection(bool showLoadDatasetButton)
    {
        ...
        ImGui.TextWrapped("This stage audits the exported dataset, writes the manifest, optionally bakes reference minimaps from the exported layers, and can also queue deterministic high-resolution WoWViewer validation captures in the same build flow.");
        ImGui.Spacing();
        ...
        ImGui.Text("WoWViewer Validation Output:");
        ImGui.SetNextItemWidth(-80);
        ...
        {
            string? result = ShowFolderDialogSTA("Select WoWViewer validation minimap output directory", _mkHarvestViewerValidationOutputDir, showNewFolderButton: true);
            if (!string.IsNullOrWhiteSpace(result))
                _mkHarvestViewerValidationOutputDir = result;
        }

        ImGui.Spacing();
        ImGui.Checkbox("Generate WoWViewer validation minimaps", ref _mkHarvestGenerateViewerValidationMinimaps);
        ImGui.Checkbox("Force rebuild existing WoWViewer validation minimaps", ref _mkHarvestForceViewerValidationRegeneration);
        ImGui.SetNextItemWidth(180f);
        ImGui.SliderInt("Validation Resolution", ref _mkHarvestViewerValidationResolution, 512, 4096);
        ImGui.TextColored(new Vector4(0.6f, 0.6f, 0.6f, 1f), "ML dataset finalize now writes the manifest and only queues live WoWViewer validation minimaps. It no longer generates baked 4k reference minimaps from exported layers.");
        ...
        if (_mkHarvestViewerValidationQueued > 0)
        {
            ImGui.TextColored(new Vector4(0.7f, 0.85f, 1f, 1f),
                $"WoWViewer validation captures: {_mkHarvestViewerValidationCompleted}/{_mkHarvestViewerValidationQueued} complete, {_mkHarvestViewerValidationFailed} failed");
        }
