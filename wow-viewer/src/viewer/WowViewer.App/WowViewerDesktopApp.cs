...
internal sealed class WowViewerDesktopApp : IDisposable
{
    ...
    private const string WindowTitle = "game-viewer";
    ...
    private bool _startupCaptureCloseRequested;
    ...
    private void OnUpdate(double deltaSeconds)
    {
        ...
        if (_startupCaptureCloseRequested && _window != null)
        {
            _startupCaptureCloseRequested = false;
            _window.Close();
            return;
        }
    ...
    private void TryCompleteStartupCapture()
    {
        ...
        if (captured && _startupCaptureRequest.ExitAfterCapture)
            _startupCaptureCloseRequested = true;
    }
    ...
    {
        ImGui.TextWrapped("`game-viewer` is the active shell for bounded viewer/runtime proof work on top of `wow-viewer`. The diagnostics panel is the main place for detailed technical state now.");
        ImGui.Separator();
    ...
    private void DrawWorldMinimapSurface(Vector2 origin, Vector2 extent, string mapName, WorldMinimapRenderer minimapRenderer)
    {
        ...
        HashSet<int> occupiedTileIndices = GetWorldMinimapOccupiedTiles()
            .Select(static tile => (tile.TileY * 64) + tile.TileX)
            .ToHashSet();
