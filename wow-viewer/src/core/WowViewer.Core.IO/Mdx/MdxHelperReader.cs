...
public static class MdxHelperReader
{
    ...
    public static MdxHelperFile Read(Stream stream, string sourcePath = "<memory>")
    {
        ArgumentNullException.ThrowIfNull(stream);
        ArgumentException.ThrowIfNullOrWhiteSpace(sourcePath);

        if (!stream.CanSeek)
            throw new ArgumentException("MDX helper reading requires a seekable stream.", nameof(stream));
        ...
        try
        {
            ...
            while (stream.Position <= stream.Length - ChunkHeader.SizeInBytes)
            {
                ...
                {
                    helpers = ReadClassicHelpers(stream, dataOffset, header.Size, version, pivotPoints);
                }

                stream.Position = endOffset;
            }

            if (helpers.Count > 0 && pivotPoints.Count > 0)
                helpers = AssignDeferredPivots(helpers, pivotPoints);

            return new MdxHelperFile(sourcePath, signature, version, modelName, helpers);
        }
        finally
        {
    ...
    private static List<MdxHelper> ReadClassicHelpers(Stream stream, long dataOffset, uint size, uint? version, IReadOnlyList<MdxPivotPointSummary> pivotPoints)
    {
        ...
        try
        {
            ...
            List<MdxHelper> helpers = new(checked((int)helperCount));
            for (int index = 0; index < helperCount; index++)
            {
                ...
                (string name, int objectId, int parentId, uint flags, MdxVector3NodeTrack? translationTrack, MdxQuaternionNodeTrack? rotationTrack, MdxVector3NodeTrack? scalingTrack) =
                    ReadNodeTracks(stream, nodeEnd, index, "HELP(v1300)");

                Vector3 pivotPoint = objectId >= 0 && objectId < pivotPoints.Count
                    ? pivotPoints[objectId].Position
                    : Vector3.Zero;

                helpers.Add(new MdxHelper(index, name, objectId, parentId, flags, pivotPoint, translationTrack, rotationTrack, scalingTrack));
            }
    ...
    private static List<MdxHelper> AssignDeferredPivots(IReadOnlyList<MdxHelper> helpers, IReadOnlyList<MdxPivotPointSummary> pivotPoints)
    {
        List<MdxHelper> reassigned = new(helpers.Count);
        foreach (MdxHelper helper in helpers)
        {
            Vector3 pivotPoint = helper.ObjectId >= 0 && helper.ObjectId < pivotPoints.Count
                ? pivotPoints[helper.ObjectId].Position
                : helper.PivotPoint;

            reassigned.Add(new MdxHelper(
                helper.Index,
                helper.Name,
                helper.ObjectId,
                helper.ParentId,
                helper.Flags,
                pivotPoint,
                helper.TranslationTrack,
                helper.RotationTrack,
                helper.ScalingTrack));
        }
