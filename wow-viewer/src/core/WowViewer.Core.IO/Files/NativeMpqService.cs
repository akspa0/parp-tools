...
/// </summary>
public class NativeMpqService : IArchiveCatalog
{
    ...
    public IReadOnlyList<string> ExtractInternalListfiles()
    {
        ...
        foreach (var archive in _archives)
        {
            ...
            if (listfileBlock == null || listfileBlock.FileSize == 0) continue;
            var data = ReadFileFromArchive(archive, listfileBlock, "(listfile)");
            if (data == null || data.Length == 0) continue;
            var content = Encoding.UTF8.GetString(data);
            foreach (var line in content.Split(['\n', '\r'], StringSplitOptions.RemoveEmptyEntries))
            {
                var path = line.Trim().Replace('/', '\\').TrimStart('\\');
                if (!string.IsNullOrEmpty(path)) allFiles.Add(path);
            }
        }
    ...
    {
        var allFiles = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        foreach (var kvp in _hashToName) allFiles.Add(kvp.Value);
        return allFiles.ToList();
    }

    public IReadOnlyList<string> ScanWmoMpqArchives(string gamePath) => Array.Empty<string>();
