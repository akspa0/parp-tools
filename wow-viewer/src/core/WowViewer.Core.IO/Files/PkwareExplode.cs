...
public static class PkwareExplode
{
    ...
    public static byte[]? Decompress(byte[] input, uint expectedSize)
    {
        ...
        try
        {
            int compType = input[0];
            int dictSizeBits = input[1];

            if (compType != CmpBinary && compType != CmpAscii)
                return null;

            if (dictSizeBits is < 4 or > 6)
                return null;
            ...
            if (compType == CmpAscii)
            {
                characterBits = ChBitsAsc;
                characterCode = ChCodeAsc;
            }
            else
            {
                characterBits = new byte[256];
                characterCode = new ushort[256];
                for (int i = 0; i < 256; i++)
                {
                    characterBits[i] = 8;
                    characterCode[i] = (ushort)i;
                }
            }

            while (outputPosition < expectedSize)
            {
                while (bitCount < 16 && inputPosition < input.Length)
                {
                ...
                int flagBit = bitBuffer & 1;
                bitBuffer >>= 1;
                bitCount--;

                if (flagBit == 0)
                {
                    if (compType == CmpBinary)
                    {
                        while (bitCount < 8 && inputPosition < input.Length)
                        {
                            bitBuffer |= input[inputPosition++] << bitCount;
                            bitCount += 8;
                        }
                        ...
                        output[outputPosition++] = (byte)(bitBuffer & 0xFF);
                        bitBuffer >>= 8;
                        bitCount -= 8;
                    }
                    else
                    {
                        int decoded = DecodeLiteral(ref bitBuffer, ref bitCount, ref inputPosition, input, characterBits, characterCode);
                        if (decoded < 0)
                            break;

                        output[outputPosition++] = (byte)decoded;
                    }
                }
                else
                {
                    ...
                    int lengthIndex = DecodeValue(ref bitBuffer, ref bitCount, ref inputPosition, input, LenBits, LenCode, 16);
                    if (lengthIndex < 0)
                        break;

                    int extraBits = ExLenBits[lengthIndex];
                    int length = LenBase[lengthIndex] + 2;

                    if (extraBits > 0)
                    {
                        while (bitCount < extraBits && inputPosition < input.Length)
                        {
                            bitBuffer |= input[inputPosition++] << bitCount;
                            bitCount += 8;
                        }

                        if (bitCount < extraBits)
                            break;

                        length += bitBuffer & ((1 << extraBits) - 1);
                        bitBuffer >>= extraBits;
                        bitCount -= extraBits;
                    }
                    ...
                    int distanceIndex = DecodeValue(ref bitBuffer, ref bitCount, ref inputPosition, input, DistBits, DistCode, 64);
                    if (distanceIndex < 0)
                        break;

                    int distance = distanceIndex << dictSizeBits;

                    while (bitCount < dictSizeBits && inputPosition < input.Length)
                    {
                        bitBuffer |= input[inputPosition++] << bitCount;
                        bitCount += 8;
                    }

                    if (bitCount < dictSizeBits)
                        break;

                    distance |= bitBuffer & ((1 << dictSizeBits) - 1);
                    bitBuffer >>= dictSizeBits;
                    ...
                    for (int i = 0; i < length && outputPosition < expectedSize; i++)
            ...
            byte[] partial = new byte[outputPosition];
            Array.Copy(output, partial, outputPosition);
            return partial;
        }
        catch
        {
    ...
    private static int DecodeLiteral(ref int bitBuffer, ref int bitCount, ref int inputPosition, byte[] input, byte[] characterBits, ushort[] characterCode)
    {
        for (int i = 0; i < 256; i++)
        {
            int bits = characterBits[i];
            if (bits == 0)
                continue;

            while (bitCount < bits && inputPosition < input.Length)
            {
                bitBuffer |= input[inputPosition++] << bitCount;
                bitCount += 8;
            }
            ...
            int mask = (1 << bits) - 1;
            if ((bitBuffer & mask) != characterCode[i])
                continue;
    ...
    private static int DecodeValue(ref int bitBuffer, ref int bitCount, ref int inputPosition, byte[] input, byte[] codeBits, byte[] codeValues, int tableSize)
    {
        for (int i = 0; i < tableSize; i++)
        {
            ...
            int mask = (1 << bits) - 1;
            if ((bitBuffer & mask) != codeValues[i])
                continue;
