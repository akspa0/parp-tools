...
internal static class WmoV17ToV14Command
{
    public static void Run(string[] args)
    {
        ...
        {
            Console.Error.WriteLine("Error: convert-wmo-v17-to-v14 requires --input-root <root.wmo> and --output <output.wmo>.");
        ...
        try
        {
            Console.WriteLine("WowViewer.Tool.Converter convert-wmo-v17-to-v14 report");
            Console.WriteLine($"  InputRoot:  {fullInputRoot}");
            Console.WriteLine($"  Output:     {fullOutputPath}");
            Console.WriteLine($"  GroupsDir:  {fullGroupsDirectory ?? Path.GetDirectoryName(fullInputRoot) ?? "."}");

            WmoV17ToV14Converter.Convert(fullInputRoot, fullOutputPath, fullGroupsDirectory);
