...
string[] tail = args.Skip(1).ToArray();
	...
	case "terrain-weak-signal-patch":
		...
		break;
	case "terrain-stratigraphy-patch":
		TerrainStratigraphyPatchCommand.Run(tail);
		break;
	...
	case "convert-alpha-to-lk":
		...
		break;
	case "convert-split-adt-to-lk":
		SplitAdtToLkCommand.Run(tail);
		break;
...
static void ShowUsage()
{
	...
	Console.WriteLine("  wowviewer-converter convert-lk-to-alpha --input <dir> --output <output.wdt> [--output-wdl <output.wdl>] [--asset-root <dir> ...] [--tileset-provenance-report <report.json>] [--no-bundle-tilesets|-nbt] [--bundle-m2s|-bm] [--bundle-wmos|-bw] [--verbose|-v]");
