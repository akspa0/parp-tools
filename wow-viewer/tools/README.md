...
|---|---|---|
| `inspect/` | `WowViewer.Tool.Inspect` | Multi-format inspection and research commands: `archive`, `assets`, `wtf`, `audio`, `blp`, `m2`, `mdx`, `map`, `adt`, `lit`, `light`, `pm4`, `pd4`, `wmo`, `rosetta-generate`, `casc` (CASC installs: `products`, `read`, `exists`, `wmo`, `m2`, `db2`, `map-survey`, `adt-heights`, `bench`) and `adt-ahdr check|objects|roundtrip` (DAT v26 terrain). |
