...
| **155 WMO doodad inventory / asset chronology** | **NEW — spec drafted, not planned** | Corpus enumeration is P1 and blocks everything: 0.5.3 holds **532** WMOs (one per-WMO `.MPQ` container under the loose `Data\World` tree) and the listfile path reports **1**. Inventory over that would date every asset too late. |
