...
| **155 WMO doodad inventory / asset chronology** | **NEW — spec drafted, not planned** | Build the inventory on the existing data-access layer (archive catalogue / data source / native archive service), which already reads 0.5.3's 532 per-asset `.wmo.MPQ` containers. **Never** enumerate the corpus from archive internal listfiles — those containers carry none, so that surface returns 1 of 532 and would date every asset too late. |
