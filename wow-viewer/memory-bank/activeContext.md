...
- Canonical target is explicit: consumers should load the decoded V22 Zarr dataset instead of the game client whenever the dataset exists. The C# harvester only emits the binary V22 stream. Zarr I/O is done by the Python package; there is no C# Zarr implementation and we are not building one.
