...
**Last updated**: 2026-07-03 | **Focus**: Spec 089 `089-dav2-height-predictor` now has a complete Speckit planning pack, a validated local Phase 0 environment, and a validated synthetic Phase 1 dataset slice (`channels.py`, `dataset.py`, `build_tileset_prune_table.py`, `tests/v23`). The next open gate is Phase 1 real-data proof (T012) on a V22 store.
...
- Phase 0 source wiring landed under `data-harvester/src/harvester/v23/`, and the public import surface still uses placeholder model/checkpoint/inference symbols for later phases. `pyproject.toml` now declares `peft` and `bitsandbytes`; `transformers>=4.52` already satisfied the plan.
- Phase 1 source wiring is also present: `channels.py` builds the 15-channel input tensor and degrade modes, `dataset.py` adapts V22 tiles into V23 samples with bicubic liquid override and channel-valid masks, `build_tileset_prune_table.py` emits the prune-table JSON, and `tests/v23/test_channels.py` + `test_dataset.py` cover the synthetic contracts. Syntax-only `py_compile` passed on these files.
- `specs/089-dav2-height-predictor/tasks.md` now has T001-T011 complete except T012. Phase 0 was proven with `uv sync`, `uv run python -c "import harvester.v23"` -> `import-ok`, and bare `uv run pytest -q` -> `309 skipped`. Phase 1 synthetic proof passed with `uv run pytest tests/v23/test_dataset.py tests/v23/test_channels.py -m v23 -q` -> `10 passed`.
...
- **Current real-data blocker**: the upstream V22 implementation gap is no longer "tool won't build." `WowViewer.Tool.V22Enrich` now builds, `build_v22_dataset.py enrich` works on staged `3_3_5_12340`, and `v22_zarr_io.py` now ingests real enrichment streams, computes `mcly_tileset_ids`, writes model/tileset libraries, and emits audit sidecars + `finalization.json`. The remaining blocker is canonical proofing: `wow-viewer/output/datasets/v22/3_3_5_12340.zarr` still does not exist, and pushing the full existing 3.3.5 V18 store through the rewritten V22 build timed out in a quick local smoke.
- **Current proof**: a bounded real-data temp proof now exists at `wow-viewer/data-harvester/tmp/v22_smoke/3_3_5_12340_tile0.zarr`, built from a one-tile temp V18 subset at `wow-viewer/data-harvester/tmp/v18_smoke/3_3_5_12340_tile0.zarr` plus `tmp/v22_smoke/3_3_5_12340_tile0.bin`. `build_v22_dataset.py stats` reports `tile_count = 1`, `model_count = 1`, `tileset_count = 9`; `inspect_v22_dataset.py tile --tile-index 0` shows non-zero terrain/minimap arrays, populated `mcly_tileset_ids`, and one MODF placement resolved into the model library.
- **Next gate**: record this repaired V22 state in Spec 088 continuity, then decide whether to (a) codify the temp-one-tile-subset proof as the Phase 9 bounded route, or (b) add a first-class bounded V18 export path so the canonical V22 proof can land under `wow-viewer/output/datasets/v22/` without rebuilding the full 3.3.5 store. Only after that should Spec 089 Phase 2 encoder work begin.
...
- **Implementation status (2026-07-03)**: the C# enrich tool now compiles against current M2/WMO APIs, decodes BLP tilesets, and emits real enrichment streams. The enrichment stream parser bug is fixed (`ENTRY` is a 5-byte magic on both writers/readers), and the Python writer no longer depends on `pandas`, no longer drops the last tile's placement rows, remaps `mcly_texture_ids` → `mcly_tileset_ids` from `decoded_metadata_json.mcly_texture_names`, and writes `index.parquet`, `placements.parquet`, `asset_inventory.parquet`, and `finalization.json` sidecars. Tileset dataset identities now use PNG-style decoded-texture keys while preserving original BLP `source_path`, `asset_inventory.parquet` now records `source_in_listfile` / `source_kind` so model and tileset rows keep archive-listfile provenance instead of looking like loose assets, and the enrich/build path now carries FR-008/009 string surfaces too: M2 `texture_paths`, WMO `material_texture_paths`, and WMO `doodad_set_paths` are serialized through the enrichment stream and materialized into the final Zarr model entries. Focused pytest now passes on `tests/test_v22_zarr_io.py` and `tests/test_v22_patched_signals.py`.
- **Current proof state**: full-store quick smoke on the existing `output/datasets/v18/3_3_5_12340.zarr` still timed out, so the current real-data proof uses a temp one-tile V18 subset under `data-harvester/tmp/v18_smoke/`. That bounded proof is enough to show the repaired V22 route is real, but it is not yet the final canonical Phase 9 operator path.
- **Next gate**: promote the bounded proof into a durable script/operator path, then rerun against the three scoped builds (`3_3_5_12340`, `0_5_3_3368`, `4_0_0_11927`) for the final Spec 088 signoff.
