...
**Last updated**: 2026-07-04 | **Focus**: Spec 089 `089-dav2-height-predictor` is source-complete locally, and the next proof owner is a real local 12 GB CUDA envelope, not RunPod.

## Current V23 Slice

- **Spec 089 `089-dav2-height-predictor` is the active V23 design.** It consumes the canonical Spec 088 V22 Zarr stores and defines one single-signal height model: DepthAnything-V2-Small encoder + LoRA-r16 + compact DPT-style height head + affine anchor.
- The Speckit planning pack is complete, the repo-local architecture note now exists at `docs/architecture/v23-height-predictor-2026-07-03.md`, and `tasks.md` has been updated to reflect the real local implementation boundary rather than stopping at Phase 3.
- Phases 0-3 are locally proven: package wiring, V22-backed channel/dataset contract, DA-V2-Small encoder wrapper, LoRA surface, height head, and model composition.
- Phases 4-6 are now also locally proven: `losses.py` implements affine / gradient / SDC / GPCT / bias-free masking, `train_v23_height.py` provides deterministic smoke training and checkpointing, and `infer_v23_height.py` plus `inference.py` provide deterministic inference plus CAI-style stitched output.
- Phase 7 is now source-complete locally: `runpod/v23/{install_deps,verify_bundle,smoke,train}.sh` exist, and `scripts/package_v23_runpod.py` builds a BYOD tar that copies the actual V23 dependency surface (`v23/` plus the V22 reader seam), optional `tests/v23`, a bounded V22 subset store, and a clean `manifest.json` with `contains_game_client_files = false`.
- External bootstrap surface now also exists: `scripts/setup_v23_runpod.py` reuses the proven Spec 079 RunPod creation logic but points it at the V23 bundle and `runpod/v23/*` bootstrap path. Real proof: `v23_smoke_bundle3.tar` was packaged, a Pod-local fallback Pod was created successfully, and the current REST state reported a public IP plus SSH port mapping.
- **Current local proof owner**: `uv run python -m pytest tests/v23 -m v23 -q` now passes with `28 passed, 14 warnings`. That suite covers dataset/channel contract, encoder/head/model, losses, deterministic smoke training, deterministic inference, CAI stitching, and bundle packaging.
- The trainer now has a real local memory-budget path: `train_v23_height.py` applies `memory-profile` rules, honors `grad_accum_steps`, records `peak_vram.json`, and degrades OOM retries by batch size / GPCT-K / AMP mode. The default target is now 12 GB, not 22 GB.
- If the remote path is reopened later, `setup_v23_runpod.py` now injects a preferred GPU order of `3090 -> 4090 -> 5090` into the shared RunPod selector. The shared helper still knows how to sort by cost/VRAM, but V23 no longer defaults to picking cards like `A4500` ahead of those consumer targets.
- Environment repair notes: the stale `.venv` was quarantined to `.venv.broken-20260703`, a fresh `.venv` was created on `C:\Python314\python.exe`, and `pyproject.toml` now includes the missing `setuptools` + `src/` package metadata so `uv run` can import `harvester` without manual `PYTHONPATH` hacks.
- Supporting unblocker: `src/harvester/v22_zarr_io.py` was missing `from dataclasses import dataclass`; that import is now present so V23 can import the V22 reader.
- **Current real-data state**: the upstream V22 implementation gap is no longer "tool won't build." `WowViewer.Tool.V22Enrich` builds, `build_v22_dataset.py enrich` works on staged clients, and canonical V22 stores now exist at `wow-viewer/output/datasets/v22/0_5_3_3368.zarr` and `wow-viewer/output/datasets/v22/3_3_5_12340.zarr`.
- **Canonical V22 contract correction**: the datastore is now `paths_only`, not a giant embedded asset blob. The store keeps the root arrays, native placement arrays, `models/model_paths` + `model_kind` + `load_error`, `tilesets/tileset_paths` + `texture_shape` + `load_error`, `mcly_tileset_ids`, and provenance sidecars (`asset_inventory.parquet` with `source_path`, `source_in_listfile`, `source_kind`). M2/WMO/BLP payloads are not part of the canonical V22 training store contract.
- **Next gate**: T035 local first. Run the 12 GB CUDA smoke profile on a real local card, record `peak_vram.json`, and only then revisit any remote retry. Current remote context remains useful only as background: network-volume creation failed because the account balance is below the provider's `$5` threshold, and the one created Pod was not accepted as a proof owner for this lane.
...
- **Architecture**: V18 Zarr store is the substrate, untouched. `WowViewer.Tool.V22Enrich` still reads the V18 store's `placements.parquet`, walks unique asset paths, and decodes each M2 / WMO / BLP exactly once via the existing C# readers, but the canonical V22 `build` step now materializes a paths-only datastore: path/id inventories, remap arrays, texture shape metadata, and provenance sidecars. The 5 V22-patched signals (`mcnr_mask_257`, `liquid_type_256`, `ground_intent_height_257`, `model_focus_mask`, `model_above_terrain_mask`) are derived in pure Python, matching the C# reference algorithms in `RawArraySerializer.cs`.
...
- **Implementation status (2026-07-03)**: the C# enrich tool compiles against current M2/WMO APIs, decodes BLP tilesets, and emits real enrichment streams. The enrichment stream parser bug is fixed (`ENTRY` is a 5-byte magic on both writers/readers), and the Python writer no longer depends on `pandas`, no longer drops the last tile's placement rows, remaps `mcly_texture_ids` → `mcly_tileset_ids` from `decoded_metadata_json.mcly_texture_names`, and writes `index.parquet`, `placements.parquet`, `asset_inventory.parquet`, and `finalization.json` sidecars. Tileset dataset identities now use PNG-style decoded-texture keys while preserving original BLP `source_path`, `asset_inventory.parquet` records `source_in_listfile` / `source_kind`, and canonical stores finalize with `asset_payload_mode = "paths_only"`. Focused pytest passes on `tests/test_v22_zarr_io.py` and `tests/test_v22_patched_signals.py`.
- **Current proof state**: canonical `build_v22_dataset.py stats` now reports `0_5_3_3368.zarr -> tile_count 1629, model_count 368, tileset_count 322` and `3_3_5_12340.zarr -> tile_count 5134, model_count 7660, tileset_count 748`, both with `missing_components = []`.
- **Next gate**: rerun the same proof for `4_0_0_11927`, then close Spec 088 and keep moving in Spec 089.
