...
## 2026-09-06 — Spec 223 operator acceptance failed; source remediation built, runtime gate reopened
- **Reported acceptance failures:** Fog Start/Fog End rubberbanded; a WMO-only map did not render its
  global WMO; Archaeology > Playback & Capture lacked the video-recording, camera-path playback, and
  apply actions implied by its text; and the existing camera rig was decorative rather than the
  requested interactive 3D HUD.
- **Source remediation:** every fog editor now reads/writes the `WorldScene` override; WMO-only global
  MODF instances enter the external admission collector without touching readers; Playback & Capture
  renders canonical Capture Automation / Camera Path controls and an explicit **Apply playback to next
  capture** action. Scene-only recording no longer hides UI chrome, and recording cleanup stops only
  archeology playback it owns. The decorative rig follows Tab but is explicitly not interactive UI.
...
## 2026-09-05 — Spec 223 Phase 6 source gate passed; interactive operator gate remains
