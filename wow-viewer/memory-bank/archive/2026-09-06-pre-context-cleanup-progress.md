...
- **Next:** T002 completed the source reconciliation; T003 is now the operator's current-build
  screenshot/input matrix. Only then select one Archaeology or Editor surface for a
  presentation-only, build-validated source slice. Spec 223 Phase 6 retest remains separately
  operator-owned.
