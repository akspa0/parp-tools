...
- **Operator acceptance failed:** Fog Start/Fog End rubberbanded; WMO-only maps did not render their
  global WMO; Archaeology > Playback & Capture exposed no record/path-playback/apply actions; and the
  visible camera rig was not the requested interactive 3D spatial UI.
- **Source remediation built:** fog controls now share `WorldScene` override state; WMO-only global
  MODF placements use the external WMO admission collector without modifying readers; Playback &
  Capture hosts the canonical Capture Automation / Camera Path controls and an explicit one-shot
  apply action; scene-only recording no longer changes UI-chrome state; recording stops archeology
  playback only when it started that session. The decorative rig now hides with Tab.
- **Verification boundary:** isolated viewer Debug build passed with 0 errors; focused WDT/WMO-policy
...
- **Next implementation lane:** Spec 212 Phase 1 (`212-T101..T104`) must deliver an interactive
  camera-space UI foundation. Existing OpenSCAD meshes are decorative and are not a substitute for
  surfaces, hit testing, pointer routing, or the Museum/time controls in later phases.
