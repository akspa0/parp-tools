...
- **2026-06-10 PM4 multi-instance click freeze.** `WorldScene.TryGetSelectedPm4ObjectGraphInfo` was rebuilding the per-selection graph on every ImGui frame: walking the entire `_pm4TileObjects` dictionary plus a 3-level GroupBy/OrderBy/ToList LINQ chain. For a single PM4 object (1-5 group members) this was sub-millisecond. For a multi-instance container (same CK24 across hundreds or thousands of parts), it was 30s per click. Added a small cache field keyed on `(selectedObjectKey, selectedGroupKey, _pm4SplitCk24ByMscnRef, _pm4SplitCk24ByConnectivity)` so the rebuild only happens when the selection or the split flags change. Same out-parameter contract, same result, same caller behavior; the only difference is that the second and subsequent ImGui frames for the same selection no longer recompute. Build green. The PM4 24-bit CK24 bit-layout (`0xAABBCC` with `00` low-byte padding) and the user's hypothesis that `0xBB` and `0xCC` may be two distinct 8-bit indices into the unknowns/subtype tables are slated for spec 058 (PM4 Scene Graph panel + semantics).
