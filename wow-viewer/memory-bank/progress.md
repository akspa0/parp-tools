...
- **Root-cause diagnostic (`scripts/diagnose_v25_height_signal.py`, ran CPU on the two-era store):** the heightmap is two signals a top-down minimap treats oppositely. ABSOLUTE per-tile elevation has std ~355–410 across tiles and sits ~322 units off-distribution on held-out Northrend (arbitrary WoW map Z origins) → physically invisible top-down → **not learnable from a minimap**. Terrain SHAPE (height minus its own per-tile mean) has relief MAE ~49–91 and is *consistent* across train(89)/val(91)/era(49) → visible as minimap texture/shading → **learnable and map-transferable**. On validation, absolute-datum error (322) is 3.5× the relief signal (91). **The whole H0/H1/H2 residual cascade and its "beat baseline by 20%" gate were grading the model against the impossible absolute sub-problem.** This is why H1 could never pass. Report: `output/analysis/spec102_minimap_baseline_v1/height_signal_decomposition.json`.
- **Direct model built (not yet trained — user rejected the GPU run pending the better data source below):** `harvester/v25/direct_shape.py` `DirectShapeUNet` (small deterministic conv U-Net, 1.93M params, output zero-meaned by construction so it can only learn shape) + `scripts/train_v25_direct_shape.py` (input `clean_minimap_256` → detrended 256×256 height; masked-L1; reports held-out shape MAE vs flat-shape floor + predicted/true shape correlation; renders side-by-side proof PNGs). Ready to run; keep for comparison.
- **H1 dead-ended at 5 attempts** (v1 defaults 223, v2 optimization-stability 222, v3 128px input 217, v4 frozen timm mobilenetv3_small_050 texture features 214.6, v5 real neighboring-tile context 215.9). All improved train loss while held-out val stuck ~214–223 vs required ≤175. The user correctly flagged that single-isolated-tile inference was a red flag I under-weighted; v5 fixed it (adjacency lookup + 4-neighbor context encoder, verified against real data) and it *still* didn't pass — consistent with the diagnostic: the gate itself was the problem, not context.
- **NEW DIRECTION (user-directed) — synthetic terrain-shadow captures as the clean shape signal:** WoWViewer's CLI capture automation (`ViewerApp_CaptureAutomation.cs`, already forces a deterministic validation sun for top-down shading, and already has noliquids/noobjects/objectsonly variants + `IgnoreTerrainHolesGlobally`) will be extended to render **terrain-shadow-only** tiles: disable liquids/wl*/textures/objects, holes OFF (→ pre-inpainted, no gaps), keep only the terrain diffuse shading + MCSH shadow map. That render is essentially a hillshade — a near-direct visual encoding of terrain slope/shape. Holes masks are a separate signal for a holes loss if needed.
  - **Concrete change point identified:** `Terrain/TerrainRenderer.cs` fragment shader already computes `lighting = uAmbientColor + uLightColor * abs(dot(norm, uLightDir))` (the shape-from-shading term) then multiplies it by diffuse textures. Need: a flat-albedo "shade-only" mode (uniform `uShadeOnly`) that keeps `lighting` × MCSH shadow, skips diffuse-texture sampling + MCCV tint. Then a `ShadeOnly` bool on TerrainRenderer (+ VLM terrain renderer), set/restored in `ApplyCaptureRequestSceneOverrides`, exposed as a new capture variant sub-folder (e.g. `terrainshadow`). Everything else (liquids/objects/holes off) already wired.
...
- **2026-07-12 architecture reset**: prior V25 implementation and run claims are invalidated as product proof. The actual deployment contract is RGB minimap tiles only, while the old plan mixed in WDL assumptions and premature multi-head work. Spec/plan/tasks were rewritten around a frozen held-out-map baseline gate followed by H0 only (one RGB-derived tile-offset residual). `train_v25_decompiler.py` now fails before loading data or initializing CUDA. No further GPU run is authorized until the deploy-input audit and immutable baselines exist.
- **Modular pipeline correction (user-directed)**: the reset itself still described a combined height+uncertainty model, violating the established small-model rule. Spec 102 now permanently prohibits multi-head/joint training and defines separate residual stages: H0 offset, H1 coarse relief, H2 detail, H3 border correction, U1 uncertainty. Each owns one output, optimizer, checkpoint, metric history, and three-epoch gate; no downstream work begins before the prior stage freezes successfully.
- **Phase 0 baseline landed without GPU use**: `audit_v25_minimap_contract.py` creates a hashed, complete-map/era split and evaluates deployable baselines with sequential Zarr reads. Report: `output/analysis/spec102_minimap_baseline_v1/baseline_report.json`; 2,381 train / 423 Northrend validation / 777 full-era test. Best L1 is 308.889 validation (RGB-flat) and 197.605 era test (train-global mean). Historical 190.31 is non-comparable. Target-derived per-tile means and WDL/height priors are prohibited.
- **H0 v1 failed and was corrected**: v1 (`h0_offset_v1`) reached validation offset MAE 321.3856, worse than RGB-flat 289.4451, because it was anchored to train-global mean instead of being a residual over the best deployable baseline. H0 v2 now starts at RGB-flat (zero residual), predicts only a world-unit correction with normalized MSE, and defaults to batch 32 for more steps in the same <=3 epochs. Gate remains <=231.5561; H1 blocked.
- **H0 v2 passed and froze**: best validation offset MAE 178.4316, era MAE 169.1934, required <=231.5561, peak VRAM 0.0905 GB. Frozen owner: `output/train_v25_h0/h0_offset_v2_rgb_residual/checkpoint_best.pt`.
- **H1 isolated trainer ready**: `H1CoarseReliefModel` consumes RGB plus the frozen H0 scalar plane and emits only a 33x33 residual. The CUDA-only trainer records the H0 checkpoint hash, caps decisions at three epochs, and gates on 20% improvement over the frozen H0 plane. WDL height is target-only. Focused modular-contract suite: 13 passed.
- **H1 R014 attempted, gate not met — stopped after three bounded, cheap tries (not open-ended thrashing).** Required `<=175.2267` (20% under the frozen H0 flat-plane baseline `219.0334`, validation-map split).
  - v1 (defaults, lr=1e-3): `223.2649` best val, oscillating (`223.26→227.39→223.87`) while train MAE improved monotonically (`202.0→191.9→183.4`) every epoch — a train/val gap present from epoch 1, not a late-training collapse.
  - v2 (lr=3e-4, weight_decay=1e-4, grad-clip=1.0, 1-epoch LR warmup, x-mirror flip augmentation — targets optimization instability and limited training diversity): `222.3550` best val, monotonic but barely moved. Ruled out instability as the primary cause.
  - v3 (128x128 minimap input instead of 64x64 — targets information loss from 4x mean-pooling washing out cliff/texture edges before the conv net ever sees them; `frozen_h0_means` block-mean-pools back down to exactly 64x64 before calling the frozen H0 checkpoint, so H0's evaluation stays bit-reproducible on its trained resolution): `216.6255` best val — the first variant to beat the raw baseline at all (219.03), by ~1%. Still 41.4 world units short of the required 175.23.
  - v4 (user-directed HF search — frozen `timm` `mobilenetv3_small_050.lamb_in1k` stage-1/stride-4 ImageNet features, 272K frozen/non-trainable params, fused into the still-tiny trainable head; explicitly NOT a depth model, never predicts height, always forced to `.eval()` regardless of the parent's train/eval state so its BatchNorm stats never drift off pretrained values): `214.6247` best val, era `171.1042` (best era result of any attempt). Confirmed real: `pretrained_texture_weights_loaded: true` in config, weights downloaded from HF hub and cached at `~/.cache/huggingface/hub/models--timm--mobilenetv3_small_050.lamb_in1k`.
  - **User correction (2026-07-12, sharp and correct): flagging "single isolated tile" as a footnote after four hyperparameter/feature rounds was itself the mistake** — it should have been the first hypothesis tested, not the last. v1-v4 all shared one unquestioned structural choice: the model saw exactly one 256x256 tile with a hard boundary and zero visibility past it, for a task (relief shape) where ridgelines/valleys routinely cross tile boundaries. Nothing in the spec's Input Invariant prohibits this — adjacent-tile RGB is exactly as available at deployment as the center tile's own RGB.
  - v5 (neighboring-tile context — real fix, not another small-model swap): added `(build, map, tile_x, tile_y)` adjacency lookup from the store's `index.parquet`, fetches the four axis-neighbors (x_minus/x_plus/y_minus/y_plus; missing neighbors at map edges or curated out of the corpus replicate the center tile rather than a fabricated black tile), coarsely block-mean-pools each to 32x32, and encodes them through a small conv+global-average-pool `NeighborhoodContextEncoder` (a few thousand params) into an 8-dim vector broadcast as extra constant planes — the same pattern H0's tile-mean plane already uses. `flip_augment` extended to swap the x_minus/x_plus slots and mirror each neighbor's own content correctly on horizontal flip; verified against real store data before the GPU run (an interior tile resolved all 4 neighbors, a genuine map-corner tile at (0,0) correctly resolved 0, and a synthetic marker test confirmed the flip both re-mirrors content and swaps slots correctly).
  - **v5 result: `215.8985` best val — worse than v4's `214.6247`**, despite train MAE improving furthest of any attempt (`171.56`) and era-test hitting its best value yet (`164.78`). Spatial context is real in this run (verified, not a bug) and it did not close the validation-map gap; if anything overfitting got marginally worse. Honest reading: a crude 8-dim global-average-pooled context vector discards exactly the directional information (does the terrain trend up/down toward each neighbor) that would matter most for a low-frequency relief signal — it answers "what's nearby" but not "which way does it slope." A cheaper, more targeted next idea (not tried yet, flagged not launched): 4 explicit scalar deltas (frozen-H0 neighbor mean minus center mean, one per direction) as direct gradient/trend inputs, instead of a vague RGB-derived vector.
  - **Verdict**: five attempts (four technique swaps plus one genuine structural fix) all land in a 214–230 band that never approaches the required ≤175.2267, with diminishing and now even slightly negative returns. Per R015 / plan.md Phase 2, **H2 stays blocked**; v4 remains the best-known (though non-passing) attempt. Stopped after v5 to report honestly rather than launch a sixth attempt unilaterally — this is now a decision point for the user, not a "try one more thing" situation.
...
- **Trainer contract corrected (2026-07-12).** The initial rewrite still incorrectly teacher-forced Stage B from the GT 33x33 prior by default, making its low train error meaningless for a minimap-only model. `train_v25_decompiler.py` now always routes Stage B through Stage A's learned WDL prediction; `wdl_height_33` remains a supervised target only, and `--student-prior` is removed. Training and validation therefore use the same universal path. The 50-epoch teacher-forced `two_era_v1` run has an honest universal validation measurement, but its train metrics are not comparable and it is not proof that the learned-prior route converged; its checkpoint is at most an explicitly labeled initialization. A regression test forbids a ground-truth prior pipeline input; v25 suite is 41/41 green.
- **Loss/head-audit correction (2026-07-12).** The first predicted-prior smoke exposed the next defect: unnormalised FFT loss made `height` thousands of units and drowned out other heads. FRAMER is now orthonormal; spatial height L1 is primary, normalized frequency height is 0.25 auxiliary, and the WDL term has unit weight. The trainer records all raw/weighted components per epoch and prints real gradient norms for every trainable head at logging intervals; new loss tests account for the total explicitly. A fresh short CUDA audit smoke is required before any long run.
- **CUDA audit smoke passed (2026-07-12).** `two_era_v3_loss_audit_smoke` ran the repaired contract on CUDA (19 train / 5 val): all eight audited head groups had finite non-zero gradient norms, total loss was auditable at `157.50` (spatial-height `74.26`, WDL `74.61`, frequency auxiliary `12.08`), and peak VRAM was `3.541 GB`. This closes routing/loss-scale proof, not model-quality proof.
...
- Recurated v6 from corrected liquid coverage: M0 1,901 train / 302 Northrend / 770 Alpha; H2 1,880 / 279 / 770; 490 tiles rejected at >=80% liquid coverage.
- Ran the corrected M0 decision gate on 2026-07-13. Three epochs improved validation IoU 0.1796 -> 0.2102 with decreasing loss and 2.14 GB peak VRAM, satisfying every registered extension-authorization condition. Resumed the best checkpoint to 12 total epochs in a separate immutable output. Best validation IoU reached 0.2764, but Alpha-era IoU was 0.0743 against the 0.10 gate; calibration selected threshold 0.50 without an era gain. M0 is not frozen and W1/H2 were not started.
- Replaced M0's undocumented `RGB | probability | target` validation strip with a labeled four-column operator panel. It embeds run and tile identity, per-row metrics, and a green-TP/red-FP/blue-FN agreement view. Added `render_spec102_m0_validation.py` and regenerated labeled Northrend plus Alpha-era epoch-12 panels from the frozen checkpoint.
