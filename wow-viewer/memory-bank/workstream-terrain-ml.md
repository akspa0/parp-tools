...
1,330 synthetic rows (688 Kalimdor, 642 Azeroth). This pre-Spec-133 store has `shadow_mask` plus
`height_257`, not `terrain_shadow_256`. The new `real_terrain_synthetic_zarr` builder preserves
the original `index.parquet` row index and source hash, assigns a complete-family map holdout
(Kalimdor train, Azeroth validation), and requires explicit `shadow_mask` opt-in for a
`geometry_only_diagnostic_raw_mcsh` corpus. This expands the bridge source without pretending that
...
- The full-store bridge dry run on `v50/v50.1/curriculum-0_5_3_3368-obj_v1.zarr` passed with
  1,330 synthetic rows, 688 Kalimdor train rows, and 642 Azeroth validation rows. Because the
