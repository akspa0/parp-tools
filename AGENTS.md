...
- Use `.github/prompts/wow-viewer-gpu-viewer-plan-set.prompt.md` for library-first, GPU-first viewer migration routing when the ask is world-open performance recovery, moving renderer ownership out of `WowViewer.App`, extracting terrain/object/runtime seams into `wow-viewer` libraries, and sequencing thin-host cutover.
