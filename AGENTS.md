...
1. **Correctness** — The code must do what it claims to do.
2. **Evidence** — Base decisions on verifiable data, not assumptions.
3. **Safety** — Never expose secrets, destroy data, or act without confirmation.
4. **Minimal changes** — Prefer the smallest viable change.
5. **Consistency** — Follow existing patterns, style, and conventions.
6. **Performance** — Optimize only when evidence shows a bottleneck.
...
This project uses patterns inspired by [GEPA](https://github.com/gepa-ai/gepa) (Genetic-Pareto optimization) for reflective prompt and skill improvement.

- **Actionable Side Information (ASI)**: When a plan, spec, or skill task fails, capture *why* — error messages, reasoning gaps, data mismatches — and feed that diagnosis back into the next iteration.
- **Pareto-aware selection**: When optimizing instructions, maintain candidates that excel on different subsets. Do not converge on a single "best" variant that loses edge-case coverage.
- **Skill improvement loop**: Each skill in the Codex Skill Registry should accumulate ASI over time. If a skill produces poor results, update it with diagnostic feedback from the trace.
