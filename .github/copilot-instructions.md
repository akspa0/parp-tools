...
- Before changing `wow-viewer` viewer-app shell work, workspace/session surfaces, navigator or inspector status panels, world-session UI, or legacy viewer cutover guidance, also read `gillijimproject_refactor/plans/wow_viewer_viewer_app_cutover_plan_2026-04-17.md` and `wow-viewer/docs/architecture/viewer-legacy-cutover-boundary-2026-04-17.md`.
...
- Use `.github/prompts/wow-viewer-gpu-viewer-plan-set.prompt.md` for library-first, GPU-first viewer migration routing when the ask is to stop deepening `WowViewer.App`, move terrain/object/world renderer ownership into `wow-viewer` libraries, replace slow viewer-time MPQ bootstrap paths with cached/manifest source seams, and sequence thin viewer-host cutover.
...
- Use `.github/prompts/wow-viewer-full-m2-parser-renderer-plan.prompt.md` when the user explicitly wants the full first-party M2 parser plus runtime plus renderer cutover planned as one ownership sequence, especially when the ask mentions removing `Warcraft.NET` or `WarcraftNetM2Adapter` ownership, inspecting `noggit-red`, or stopping MDX-shaped assumptions in the M2 path.
...
- Treat `wow-viewer/src/viewer/WowViewer.App` as the canonical home for new viewer-shell work, including session/workspace contracts, navigator or inspector or status surfaces, and bounded viewer-facing CLI proof paths.
...
- Do not add new long-range shell architecture, panel design, or session-state ownership to `MdxViewer` when the same seam belongs in `WowViewer.App`.
- Keep `wow-viewer/docs/architecture/viewer-legacy-cutover-boundary-2026-04-17.md` aligned with any future viewer ownership shift so later chats do not route new app work back into `ViewerApp` by default.
