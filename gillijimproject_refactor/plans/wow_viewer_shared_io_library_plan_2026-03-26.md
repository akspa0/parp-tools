...
3. deepen shared WMO ownership from root, group, `MLIQ`, `MOBA`, `MOPY`, `MOTV`, `MOCV`, and `MODR` semantic summary into the next narrow deep-payload seam only when a fixed validation target is available
