...
- Apr 25 status update:
  ...
  - Apr 26 follow-up: the host now owns a copied diagnostics/session contract via `wow-viewer/src/viewer/WowViewer.App/WowViewerWorldDiagnosticsSnapshot.cs`, and the runtime summary, diagnostics panel, world status window, and minimap loaded-tile label now read that host-owned contract instead of the raw frame/session surfaces directly
  - Apr 26 follow-up: `wow-viewer/src/viewer/WowViewer.App/WowViewerWorldAssetInventory.cs` is the first host-managed asset inventory pass; the host now derives `WowViewerWorldAssetState` through that inventory so referenced asset keys and pending-load samples have an owner seam beyond the raw frame
  - Apr 26 follow-up: the asset inventory now owns queue-shaped WMO/MDX family state, including priority-pending and ready asset-family counts, so host asset status is closer to `WorldAssetManager` semantics instead of a raw `PendingAssetKeys` mirror
  - Apr 26 follow-up: `wow-viewer/src/viewer/WowViewer.App/WowViewerWorldNavigatorState.cs` is the first host-owned object navigator/selection seam; object entry lists, default selection, and selection resolution now live in the host and the app consumes that contract for compact world-session navigation and inspector surfaces
  - Apr 26 follow-up: the host asset inventory now has a bounded `ProcessPendingLoads(...)` consumer seam, and `WowViewerWorldSceneHost.ProcessPendingAssetLoads(...)` drives that owned queue state through `ViewerIoService.TryReadVirtualFile(...)` during app update ticks
  - Apr 26 follow-up: `wow-viewer/src/viewer/WowViewer.App/WowViewerWorldSpatialSnapshot.cs` now owns planar bounds for host-side world-space mapping, and the object marker canvas/pick path consumes that spatial snapshot plus `WowViewerWorldNavigatorState` instead of reading raw-frame planar bounds and object arrays directly
  - Apr 26 follow-up: `WowViewerWorldSceneHost` now owns the terrain preview snapshot and a `RefreshRendererPreview(...)` execution seam, so `WowViewerDesktopApp` no longer calls `LoadPreview(...)` directly for world debug-setting reloads and now reads world CPU/visible status text, selected-tile minimap highlights, and staging/debug summary values from host-owned scene, diagnostics, and asset contracts
