...
- landed surface:
  ...
  - the wrapper defaults `IncludeBuilds` to `0_5_3_3368`, `0_5_5_3494`, `0_7_0_3694`, `3_0_1_8303`, `3_3_5_12340`, and `4_0_0_11927`, with early builds expected from staged roots under `output/tmp/wowarchive-clients`
  - `3_0_1_8303` now maps to `Northrend` in the direct wrapper so the core archive-backed corpus picks up the terrain family that best overlaps later development-map content
- current proof:
  ...
  - staged-WoWArchive bounded proof now also exists for `0_5_3_3368/Azeroth`: the direct `dataset-scan`, `dataset-audit`, and `dataset-build-cache` commands all completed through the reused embedded-Alpha WDT tile reader, with scan manifests carrying `Azeroth.wdt.MPQ#alpha-tile(x,y)` reopen markers
  - staged-WoWArchive bounded wrapper proof now also exists across `0_5_3_3368`, `0_5_5_3494`, and `0_7_0_3694`: `run_v9_direct_pipeline.ps1 -Mode audit -WowArchiveOnly -NoRequireMinimap -NoRequireWdl` completed end to end with `MergedSamples = 7`, `Curated = 4`, `Processed = 4`, and trainer audit `rejected = 0`
  - a later bounded dry-run proved the updated wrapper resolves staged `3_0_1_8303` and targets `Northrend` end to end through direct scan/audit/curate/build-cache/trainer-audit
  - the current GitHub-facing handoff doc is now `wow-viewer/docs/validation/direct-v9-training-setup.md`, which pairs the direct archive-backed training cache with a separate dev-eval cache built from `datasets/original_development/development`
  - that development dev-eval root already includes loose-overlay-derived object-marking surfaces on object-bearing tiles, including `object_visibility_mask`, `pm4_mask`, `no_object_minimap`, and `terrain_only_minimap`, so it should be treated as a richer held-out evaluation surface than plain minimap-only validation
  - `4_0_0_11927` also belongs in the main corpus for development-map-era supervision; the current machine proof stages the fixed local `H:/CLIENTS/World of Warcraft Cata beta 11927` root into `output/tmp/wowarchive-clients/4_0_0_11927/World of Warcraft` because the mounted WoWArchive search did not expose a `11927` folder here
  - the current live training branch is `output/ml-training/v9_direct_archive_core_devholdout_plus11927_20260420`, launched with `0_5_3_3368`, `0_5_5_3494`, `0_7_0_3694`, `3_0_1_8303`, `3_3_5_12340`, and `4_0_0_11927` plus the separate development dev-eval cache manifest
  - the later staged-WoWArchive repairs closed the `0_7_0_3694/Azeroth` direct-scan parse failure and the spaced-entry `md5translate` regression for archive-backed minimaps, so the bounded default-gate wrapper now completes end to end with `MergedSamples = 8`, `Curated = 4`, `Processed = 4`, and trainer audit `rejected = 1`
  - the remaining default-gate rejection on that bounded staged smoke is now only `0_5_5_3494:EmeraldDream_24_25 -> missing_minimap_rgb_256`; `0_7_0_3694:EmeraldDream_24_25` now resolves a real archive-backed minimap and no longer fails the trainer gate
  - later side-by-side checks proved the exact staged `0_5_5_3494/EmeraldDream_24_25` miss is not just a wow-viewer extraction defect: the current legacy `WoWMapConverter` exporter also reports `Minimap not found for EmeraldDream 24_25` on the same staged client root
  - stronger build-content evidence now says staged `0_5_5_3494` has no `map24_25.blp` entry anywhere in root `Data\textures\Minimap\md5translate.txt`; local `0.6.0` test data has an `Emerald Dream` translation block but still not that exact tile; staged `0_7_0_3694` is the first checked build here that exposes `Emerald Dream\map24_25.blp`
- current guardrail:
  - do not describe this as proof that early-build direct reads are closed under default gates: the remaining staged default-gate blocker is still `missing_minimap_rgb_256` on `0_5_5_3494:EmeraldDream_24_25`, and the current evidence points to a real build-era minimap-translation/content boundary rather than only a missed wow-viewer port
