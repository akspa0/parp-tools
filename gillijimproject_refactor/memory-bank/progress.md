...
| **V16 optional-head per-sample loss gating fix** | **DONE (IMPLEMENTED + SMOKE PROOF)** — replaced batch-mean `has_*` scaling with strict per-sample masked reduction for normals/alpha/holes/liquid/liquid-height/MCLY so missing-signal tiles no longer influence optional heads through zero-filled targets; smoke run `smoke_v16_per_sample_maskfix` completed successfully |
