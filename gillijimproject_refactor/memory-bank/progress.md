...
| **V16 index coordinate repair path** | **DONE (UNVALIDATED)** — future streamed tile metadata now carries explicit `tile_x` / `tile_y`, and `build_v16_dataset.py repair-index` can rewrite existing `index.parquet` files in place from a metadata-only re-stream instead of forcing full dataset rebuilds |
...
| **V16 liquid-height training supervision** | **SUPERSEDED** — this briefly existed during a short-lived terrain-lane experiment, but current V16 terrain training no longer predicts or supervises `liquid_height`; the dataset still retains it for a future dedicated liquid-refinement model. |
| **V16 liquid inference sidecars for ADT liquid follow-up** | **SUPERSEDED** — current core V16 inference emits `predicted_height_257.npy` and `predicted_liquid_mask_256.npy`; liquid-height sidecars were removed when `liquid_height` was deferred from the terrain lane. |
...
| **V16 optional-head per-sample loss gating fix** | **DONE (IMPLEMENTED + SMOKE PROOF)** — replaced batch-mean `has_*` scaling with strict per-sample masked reduction for normals/alpha/holes/liquid/MCLY so missing-signal tiles no longer influence optional heads through zero-filled targets; smoke run `smoke_v16_per_sample_maskfix` completed successfully |
| **V16 alpha `map=memory` metadata fix + repair fallback** | **DONE (IMPLEMENTED, DATA REPAIR PENDING)** — C# harvest now passes real alpha WDT virtual source paths when reading tiles from bytes, Python builder normalizes placeholder map names to requested map context, and `repair-index` now supports placeholder-only relabel fallback. Existing already-built alpha stores still need `repair-index` or rebuild to rewrite old `map=memory` rows. |
| **V16 curation placeholder-row exclusion** | **DONE (IMPLEMENTED + SMOKE PROOF)** — `train_v16.py` now excludes placeholder-map rows (`memory`, empty, unknown) from train/val subset curation by default, with opt-in override `--include-placeholder-map-tiles`; smoke run `smoke_v16_filter_placeholder_maps` showed `dropped_placeholder_map_tiles` counts in curation manifest. |
| **V16 long-run resume hardening** | **DONE (IMPLEMENTED + RESTART PROOF)** — `train_v16.py` now supports `--resume-from none|auto|last|best`, writes `v16_last.pt` every epoch, and persists full state (model/optimizer/scheduler/scaler/best/log). Restart smoke `smoke_v16_resume` resumed from `v16_last.pt` at epoch 2 and completed cleanly. |
| **V16 legacy Alpha tile-coordinate fallback parse** | **DONE (IMPLEMENTED + SANITY PROOF)** — `build_v16_dataset.py` now recovers missing Alpha `tile_x/tile_y` from `#alpha-tile(x,y)` markers in `tile_name`/`source_adt_path`, preventing silent `(0,0)` defaults for legacy blobs; parser sanity check returned expected coordinates. |
| **V16 liquid-height deferral from core model** | **DONE (IMPLEMENTED + FOCUSED PROOF)** — current V16 model/trainer/infer path no longer predicts or supervises `liquid_height`; dataset retains `liquid_height` for future refinement model. `validate_v16_training_ready.py` now reports `trainer_uses_liquid_height=false`; smoke run `smoke_v16_no_liquid_height` completed cleanly. |
| **V16 docs: planned separate liquid-refinement lane** | **DONE (DOC NOTE)** — README/spec now document a dedicated future liquid model (`minimap + liquid priors -> liquid mask + liquid height`) and keep terrain reconstruction separate, with liquid/object masks retained as terrain-loss gating signals. |
| **V16 liquid GT presence-mask fix (type=0 water)** | **DONE (IMPLEMENTED + TARGETED SMOKE)** — C# tensor-pack/NPZ now emits `mh2o_presence_mask` + `mclq_presence_mask`, and Python liquid derivation now prefers explicit presence, handles Alpha `mclq_type_mask=-1` sentinel via `>=0` fallback, and no longer drops valid type `0` liquids; direct smoke confirmed non-zero mask recovery for Alpha-style type-0 MCLQ. |
| **V16 dataset post-build signal-validation gate** | **DONE (IMPLEMENTED + SMOKE PROOF)** — `build_v16_dataset.py build` now writes `signal_validation.json` and (by default) fails strict on missing expected `has_*` coverage or era-specific liquid source regressions; strict smoke passed on existing `0_5_3_3368` store. |
| **V16 default lightweight Zarr codec profile** | **DONE (IMPLEMENTED)** — dataset build defaults use Blosc `lz4` level `1` with `shuffle`, preserving light Zarr chunk compression while keeping the lean raw harvest stream and avoiding NPZ/zip-heavy per-tile compression. |
| **V16 trainer validation-snapshot randomness + build-mix fix** | **DONE (IMPLEMENTED + SMOKE PROOF)** — `train_v16.py` now uses fresh random seed by default for new runs (resume reuses prior run seed), curation order is randomized (no index re-sort), validation snapshots sample per epoch from curated val data with build-balanced selection by default, and each snapshot export writes `snapshot_selection.json`; smoke run `smoke_v16_snapshot_mix` confirmed mixed-build snapshot selection (`3_3_5`, `0_5_3`, `0_7_0`, `4_0_0`). |
| **V16 in-place liquid datastore patch lane** | **DONE (IMPLEMENTED)** — `build_v16_dataset.py patch-liquids` can now re-stream and rewrite only `liquid_mask` / `liquid_height` plus liquid `has_*` provenance flags in `index.parquet`, with `index.parquet.bak.liquids` backup, `liquid_patch_report.json`, and optional strict post-patch signal validation. |
| **V16 seeded random human-eye dataset QA artifacts** | **DONE (IMPLEMENTED + SMOKE PROOF)** — `inspect_v16_dataset.py` now supports seeded random sampling and `liquid_focus` mode, writes labeled visual audit overviews (`<build>.validation_audit_overview.png`) and paired JSON sample metadata, and smoke run on `0_5_3_3368` produced expected artifacts in `output/datasets/v16/inspection/`. |
| **data-harvester README concise rewrite + advanced guide split** | **DONE (DOC)** — primary README reduced to short operator flow; detailed options/workflows moved to `data-harvester/docs/advanced-v16-workflows.md` to keep onboarding readable. |
...
| **V16 dataset/spec doc sync for expanded signal set** | **DONE (DOC)** — `v16-terrain-model-spec-2026-05-16.md`, `wow-viewer/README.md`, and `data-harvester/README.md` now match the live corpus: expanded object/liquid arrays, placement/provenance companions, light Zarr compression defaults, and the lean raw `ARRY` stream wording. |
| **V16 Cata inline-texture fallback regression fix** | **DONE (FOCUSED PROOF)** — archive-backed byte-path harvest had dropped `mcly_texture_ids`, `mcly_layer_mask`, and `mcal_alpha_pack_256` for mixed `4_0_0_11927` tiles because `_tex0` was treated as required. `AdtTensorPackBuilder.ReadTextureDataFromBytes(...)` now falls back to inline root texture parsing when `_tex0` is absent. Focused repro on staged `AhnQiraj (27,46)` now restores the missing texture signals. |
| **V16 full-store visual QA artifact pass** | **DONE (OPERATOR PROOF)** — `inspect_v16_dataset.py` ran across all six finalized stores and wrote per-build `summary.json`, `samples.json`, and `validation_audit_overview.png` files under `wow-viewer/output/datasets/v16/inspection/`. All current `signal_validation.json` files pass; only `0_7_0_3694` carries the expected allowed warning for zero `has_holes_16` coverage. |
...
| V16 harvest recovery implementation | PARTIAL OPERATOR PROOF — the recovery features are landed and no longer blocked on pure theory: rebuilt finalized stores exist for all six target builds, `4_0_0_11927` regained inline-root `MCLY`/`MCAL` via the `_tex0` fallback fix, and full-store visual QA artifacts plus passing `signal_validation.json` files now exist for the current corpus. Remaining nuance is downstream WL* fill semantics, not broken harvest truth. |
| V16 trainer-readiness proof | DONE (FOCUSED PROOF) — `validate_v16_training_ready.py` passed on staged `3_3_5_12340`, and the current six finalized stores also have passing `signal_validation.json` plus human-eye QA artifacts under `output/datasets/v16/inspection/`. |
