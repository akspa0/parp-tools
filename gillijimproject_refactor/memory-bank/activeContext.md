...
- current boundary:
	...
	- `WowViewerDesktopApp` still uses the current runtime-frame shape for object-canvas spatial mapping, debug views, and the remaining bridge-backed world metrics, and `WowViewerWorldRuntimeBridge` is still the producer of that frame
