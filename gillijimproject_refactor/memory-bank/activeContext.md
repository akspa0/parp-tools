...
### Existing Tools (wired to CLI)
- `modf-reconstruct` (alias for `pm4-reconstruct-modf`) - Performs geometry matching and coordinate transformation
- `WoWRollback.Core/Services/PM4/Pm4ModfReconstructor.cs` - Core reconstruction logic
- `WoWRollback.Core/Services/PM4/AdtModfInjector.cs` - transform logic used
...
## Update 2025-12-10 – Development Map Repair Pipeline
**Command**: `dotnet run --project WoWRollback/WoWRollback.Cli -- development-repair`

### Achievements
1.  **Smart Caching System**: 
    -   Implemented persistent JSON caches (`wmo_library.json`, `pm4_library.json`, `match_cache.json`).
    -   Reduced iteration time from 20+ minutes to seconds.
    -   Added `--cache-dir` to share heavy data across different output experiments.
2.  **Pipeline Fixes**:
    -   Fixed `SplitAdtMerger` to correctly write `sizeAlpha` (0x70) and `sizeShadow` (0x78) in MCNK headers.
    -   Fixed missing textures by ensuring correct propagation of `MTEX`/`MCLY` from `_tex0` files.
3.  **Verification & Dumping**:
    -   Added automatic verification step that checks for empty `MWMO`, `MODF`, and `MTEX` chunks.
    -   Added `--dump-objs` and automatic `chunk_dump/` output to inspect raw binary data of generated tiles.

### Critical Findings (The "Domino Effect")
-   While the *tooling* is now correct, the **source ADTs** (`PM4ADTs/combined`) being used as input are corrupted.
-   They were generated by previous iterations of the code that lacked the texture size fixes.
-   Result: `development-repair` is correctly merging *bad data*, resulting in empty/broken outputs.
-   **Resolution**: We must re-run the `wdl-to-adt` and `adt-merge` steps from scratch to generate a clean, valid set of source ADTs before running `development-repair` again.
