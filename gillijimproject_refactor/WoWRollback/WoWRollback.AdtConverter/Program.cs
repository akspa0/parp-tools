...
using WoWRollback.LkToAlphaModule.Tools;
using WoWRollback.LkToAlphaModule.LkInspectors;
...
internal static class Program
{
    private static int Main(string[] args)
    {
        ...
        var inspectLkCmd = new Command("inspect-lk", "Inspect LK ADT MCNK texture layers (MCLY/MCAL metadata)");
        var inspectAdt = new Option<string>("--adt", description: "Path to LK root ADT file (e.g., map_YY_XX.adt)") { IsRequired = true };
        var inspectTile = new Option<int>("--tile", () => -1, "Optional MCNK tile index (0-255). When omitted, all tiles are listed.");
        var inspectVerbose = new Option<bool>("--verbose", () => false, "Include per-layer offsets and alpha lengths.");
        inspectLkCmd.AddOption(inspectAdt);
        inspectLkCmd.AddOption(inspectTile);
        inspectLkCmd.AddOption(inspectVerbose);
        inspectLkCmd.SetHandler((string adtPath, int tileIndex, bool verbose) =>
        {
            try
            {
                LkAdtInspector.Dump(adtPath, tileIndex, verbose);
            }
            catch (Exception ex)
            {
                Console.Error.WriteLine($"inspect-lk failed: {ex.Message}");
                Environment.ExitCode = 1;
            }
        }, inspectAdt, inspectTile, inspectVerbose);

        root.Add(inspectLkCmd);

        var mcalDumpCmd = new Command("mcal-dump", "Summarize MCAL dump files generated by pack-monolithic (--verbose-logging)");
        var dumpRoot = new Option<string?>("--root", () => "debug_mcal", "Root directory containing MCAL dumps");
        var dumpStage = new Option<string?>("--stage", () => "alpha", "Stage prefix (alpha or lk)");
        var dumpX = new Option<int>("--x", description: "Tile IndexX (MCNK)" ) { IsRequired = true };
        var dumpY = new Option<int>("--y", description: "Tile IndexY (MCNK)" ) { IsRequired = true };
        var dumpAscii = new Option<bool>("--ascii", () => false, "Print ASCII preview for layers");
        var dumpAsciiLayer = new Option<int?>("--ascii-layer", description: "Optional specific layer index for ASCII preview");
        mcalDumpCmd.AddOption(dumpRoot);
        mcalDumpCmd.AddOption(dumpStage);
        mcalDumpCmd.AddOption(dumpX);
        mcalDumpCmd.AddOption(dumpY);
        mcalDumpCmd.AddOption(dumpAscii);
        mcalDumpCmd.AddOption(dumpAsciiLayer);
        mcalDumpCmd.SetHandler((string? rootDir, string? stage, int ix, int iy, bool ascii, int? asciiLayer) =>
        {
            try
            {
                McalDumpUtility.Dump(rootDir ?? "debug_mcal", ix, iy, stage ?? "alpha", ascii, asciiLayer);
            }
            catch (Exception ex)
            {
                Console.WriteLine($"MCAL dump failed: {ex.Message}");
            }
        }, dumpRoot, dumpStage, dumpX, dumpY, dumpAscii, dumpAsciiLayer);

        root.Add(mcalDumpCmd);
        ...
        roundTripCmd.SetHandler((string? alphaAdt, string? lkAdt, string outDir) =>
        {
            try
            {
                ...
                {
                    Console.Error.WriteLine($"✗ Round-trip test FAILED");
                    Console.Error.WriteLine($"  Error: {result.ErrorMessage}");
                    if (result.BytesDifferent > 0)
                        Console.Error.WriteLine($"  Bytes different: {result.BytesDifferent}");
                    Environment.ExitCode = 1;
                }
            }
            catch (Exception ex)
            {
                Console.Error.WriteLine($"roundtrip-test failed: {ex.Message}");
