...
internal sealed class ViewerBuildStageRunner
{
    ...
    public ViewerStageResult Run(SessionContext session, IReadOnlyList<AdtStageResult> adtResults)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrWhiteSpace(session.Options.MpqRoot))
            {
                var locales = (session.Options.MpqLocales ?? "enUS").Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries);
                var listfiles = new List<string>();
                if (!string.IsNullOrWhiteSpace(session.Options.LkListfile) && File.Exists(session.Options.LkListfile)) listfiles.Add(session.Options.LkListfile);
                if (!string.IsNullOrWhiteSpace(session.Options.CommunityListfile) && File.Exists(session.Options.CommunityListfile)) listfiles.Add(session.Options.CommunityListfile);
                // Prefer known-working WoWTools wrapper when available
                if (listfiles.Count > 0)
                {
                    providerFactory = (ver) => new WoWToolsMinimapProvider(session.Options.MpqRoot!, locales, listfiles);
                    log.WriteLine($"[viewer] mpq provider (wowtools): root={session.Options.MpqRoot} locales={string.Join(',', locales)} listfiles={string.Join(';', listfiles)}");
                }
                else
                {
                    log.WriteLine("[viewer][warn] mpq provider not enabled: no listfiles provided/found");
                }
            }
