...
# Extract first tile from WDT
$wdtPath = "J:\wowDev\parp-tools\gillijimproject_refactor\test_data\0.5.5\tree\World\Maps\Azeroth\Azeroth.wdt"
$testDir = "J:\wowDev\parp-tools\gillijimproject_refactor\WoWRollback\single_tile_test"

Remove-Item -Path $testDir -Recurse -Force -ErrorAction SilentlyContinue
New-Item -ItemType Directory -Path $testDir -Force | Out-Null

Write-Host "Testing single tile extraction and conversion..." -ForegroundColor Cyan

# Use the extractor to get one tile
$code = @"
using System;
using System.IO;
using System.Linq;
...
$code | Out-File -FilePath "$testDir\test.cs" -Encoding UTF8

# Compile and run
Push-Location $testDir
try {
    dotnet-script test.cs --no-cache
} finally {
    Pop-Location
}
