...
public class WmoPathfindingExtractor
{
    ...
    public WmoPathfindingData ExtractFromBytes(byte[] data, string wmoPath)
    {
        ...
        result.CollisionSurfaces.AddRange(groupData.CollisionSurfaces);
    ...
    private WmoPathfindingData ExtractFromData(byte[] data, string wmoPath, int groupIdx)
    {
        ...
        for (int i = 0; i + 2 < indices.Count; i += 3)
        {
            ...
            // Extract Collision Surfaces (MOPY Flag 0x20 often indicates collision, 
            // but in some versions it might be different. We assume 0x20 per plan).
            // Actually, verified WMO MOPY flags: 0x20 is usually "Detail" but 0x01 is often implicit collision?
            // Wait, USER REQUEST said "Flag 0x20 Data". I will follow the user's explicit instruction.
            if ((surface.MaterialFlags & 0x20) != 0)
            {
                groupData.CollisionSurfaces.Add(surface);
            }
        }
...
public class WmoStructure
{
    ...
    public void ComputeAggregates()
    {
        ...
        foreach (var g in Groups)
        {
            ...
            Aggregate.CollisionSurfaces.AddRange(g.CollisionSurfaces);
...
public partial class WmoPathfindingData
{
    public string WmoPath { get; set; } = "";
    public List<WalkableSurface> WalkableSurfaces { get; } = new();
    public List<WalkableSurface> WallSurfaces { get; } = new();
    public List<WalkableSurface> CollisionSurfaces { get; } = new(); // surfaces with Flag 0x20
...
public static class WmoDataExtensions
{
    public static List<Vector3> GetVertices(this List<WalkableSurface> surfaces)
    {
        return surfaces.SelectMany(s => new[] { s.V0, s.V1, s.V2 }).Distinct().ToList();
    }
}
...
{
    // Helpers
    public List<Vector3> WalkableVertices => WalkableSurfaces.SelectMany(s => new[] { s.V0, s.V1, s.V2 }).Distinct().ToList();
    public List<Vector3> AllVertices => WalkableSurfaces.Concat(WallSurfaces).Concat(CollisionSurfaces)
        .SelectMany(s => new[] { s.V0, s.V1, s.V2 })
        .Distinct()
        .ToList();
    
    public List<Vector3> CollisionVertices => CollisionSurfaces
        .SelectMany(s => new[] { s.V0, s.V1, s.V2 })
        .Distinct()
        .ToList();
}
