...
* The top-right corner of the map has X = 17066, Y = -17066
(533.3333 * 32 = 17066.6666...)
...
The [[ADT#MCNK_chunk|MCNK]] header is 128 bytes large.
 ...
 /*0x008*/  uint32_t IndexY;
   ...
   float radius;
 #endif
 /*0x00C*/  uint32_t nLayers;                              // {{Template:Sandbox/VersionRange|max_expansionlevel=12|max_exclusive=1}}: maximum 4
 ...
 /*0x040*/  uint2_t[8][8] predominantTexture;             // "predTex", "ReallyLowQualityTextureingMap", It is used to determine which detail doodads to show. Values are an array of two bit 
                                                          // unsigned integers, naming the layer.
                                                          // {{Template:Sandbox/VersionRange|min_expansionlevel=12}}: If more than four layers are used, MPTX is needed.
...
''Note: The normal vectors don't always have a length of 1. The value is close enough with the intent to be always 1, but the error is as high as 7 epsilon (1 epsilon is the error caused by the fixed signed 8-bit value over all 3 coordinates). With a resolution of 1.5 degrees per coordinate unit (180°/127) this already causes some lighting glitches in the terrain. As I've never encountered a value of +/-127 for the X and Y coordinate, the data writer seems to use truncation instead of rounding for both coordinates, but rounding for the Z coordinate. The Z-coordinate is always positive (always pointing up/outwards). That behavior would account for the found epsilon errors. Maybe it's best to recalculate Z from X and Y. (float Z = sqrt(1 - (X / 127)² - (Y / 127)²), Z >= 0). I don't know if the client re-normalizes vectors in code or in shaders. --[[User:Nieriel|Nieriel]] Aug 2019''

=== MCLY sub-chunk ===
* split files: tex
*'''Texture layer definitions for this map chunk. 16 bytes per layer.
** {{Template:Sandbox/VersionRange|min_expansionlevel=0|max_expansionlevel=11}} up to 4 layers
** {{Template:Sandbox/VersionRange|min_expansionlevel=12}} technically no limit, but as of {{Template:Sandbox/PrettyVersion|expansionlevel=12|build=12.0.0.63534}} client ignores any layer after the 8th.
...
=== MPTX sub-chunk ===
{{Template:SectionBox/VersionRange|min_expansionlevel=12}}

 struct {
   uint2_t[8][8] factor;
 } extraPredominantTextureFactors[];
 #define MD_CHUNK_PREDOMINANT_TEX_SIZE (2 * 8 * 8 / CHAR_BIT) // == 16

from what I understand now: for determining the predom texture, 
* take predTex = mcnk.predominantTexture as before.
* for every predTex[j][i], add 2 * (q+1) * extraPredominantTextureFactors[q].factor[j][i]
** i.e. predTex[j][i] = 0b01; extraPredominantTextureFactors[0].factor[j][i] = 0b11; extraPredominantTextureFactors[1].factor[j][i] = 0b01; -> tex[j][i] = 1 + 2*1*3 + 2*2*1 = 11 -> take mcly[11]. 
** this shows why there can be multiple mptx: 0b11 + 2*b11 = 9, so would cap it again.

 predTex[a][b] = mcnk.predominantTexture[a][b] + [(2 * (q + 1) * extraPredominantTextureFactors[q].factor[a][b]) for all q]
...
{{SectionBox/VersionRange|min_expansionlevel=11|min_exclusive=1|max_expansionlevel=12|max_build=12.0.0.63534}}
Blizard has not yet implemented support for more than four layers.
