...
class PlayerManager(UnitManager):
    ...
    # override
    def set_rooted(self, active=True, index=-1):
    ...
    def send_loot_release(self, loot_selection):
        ...
        if high_guid == HighGuid.HIGHGUID_UNIT or high_guid == HighGuid.HIGHGUID_PET:
            ...
            target_world_object = self.get_map().get_surrounding_gameobject_by_guid(self, loot_guid)
        elif high_guid == HighGuid.HIGHGUID_ITEM:
            target_world_object = self.inventory.get_item_by_guid(loot_guid)
        else:
    ...
    def set_death_bind(self, map_id, position):
        if not self.deathbind:
            # Create a mock object or dictionary if deathbind is None (assuming it's a DB model usually)
            # Since we can't easily instantiate the DB model here without imports, we assume it exists or use a simple class.
            class DeathBindData:
                pass
            self.deathbind = DeathBindData()
        
        self.deathbind.deathbind_map = map_id
        self.deathbind.deathbind_position_x = position.x
        self.deathbind.deathbind_position_y = position.y
        self.deathbind.deathbind_position_z = position.z
        
        # Send update to client
        self.enqueue_packet(self.get_deathbind_packet())
