...
public static partial class Pm4ResearchMscnAnalyzer
{
    ...
    public static Pm4MscnRelationshipReport AnalyzeDirectory(string inputDirectory)
    {
        ...
        var ck24SurfaceCount = new Dictionary<string, int>(StringComparer.Ordinal);
        var ck24DistinctMdosCount = new Dictionary<string, int>(StringComparer.Ordinal);
        var ck24MeshVertexCount = new Dictionary<string, int>(StringComparer.Ordinal);
        var ck24AlignmentMode = new Dictionary<string, int>(StringComparer.Ordinal);
        ...
        foreach (Pm4ResearchFile file in files)
        {
            ...
            bool hasTileCoordinates = tileX.HasValue && tileY.HasValue;
            ...
            if (hasTileCoordinates)
            {
                ...
                {
                    bool rawFits = IsWithinTileWorldBounds(point.X, point.Y, tileX!.Value, tileY!.Value);
                    bool swappedFits = IsWithinTileWorldBounds(point.Y, point.X, tileX.Value, tileY.Value);
                ...
                string dominantMode = ResolveDominantMode(fileSwapped, fileRaw, fileTileLocal, fileNeither);
                switch (dominantMode)
                {
                    case "swapped-world":
        ...
        List<Pm4MscnClusterExample> topNonZeroClusters = clusterExamples
        ...
        List<Pm4MscnClusterExample> topZeroClusters = clusterExamples
            .Where(static cluster => cluster.Ck24 == 0)
            .OrderByDescending(static cluster => cluster.DistinctMdosCount)
            .ThenByDescending(static cluster => cluster.SurfaceCount)
            .ThenBy(static cluster => cluster.SourcePath)
        ...
        List<Pm4MscnClusterExample> topInvalidMdosClusters = invalidMdosExamples
        ...
        List<string> notes =
        [
            "No external graph or geometry package was already present in the active standalone PM4 path; this analyzer reuses the older repo insight that MSUR.MdosIndex is the main bridge into MSCN.",
            "This report is intended to test whether MSCN is a missing ownership/collision layer, not to declare MSCN authoritative for final viewer reconstruction by itself.",
