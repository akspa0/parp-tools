...
public static class Pm4ResearchUnknownsAnalyzer
{
    ...
    public static Pm4UnknownsReport AnalyzeDirectory(string inputDirectory)
    {
        ...
        Pm4CorpusAuditReport chunkPopulation = Pm4ResearchAuditAnalyzer.AnalyzeDirectory(inputDirectory);
        ...
        var mismatchDomainFits = MslkMismatchCandidateDomains.ToDictionary(static name => name, static _ => 0, StringComparer.Ordinal);

        var typeFlagsCounts = new Dictionary<string, int>(StringComparer.Ordinal);
        var subtypeCounts = new Dictionary<string, int>(StringComparer.Ordinal);
        var systemFlagCounts = new Dictionary<string, int>(StringComparer.Ordinal);
        var msurIndexCountCounts = new Dictionary<string, int>(StringComparer.Ordinal);
        var msurAttributeMaskCounts = new Dictionary<string, int>(StringComparer.Ordinal);
        var msurGroupKeyCounts = new Dictionary<string, int>(StringComparer.Ordinal);
        var mshdField00Counts = new Dictionary<string, int>(StringComparer.Ordinal);
        var mshdField04Counts = new Dictionary<string, int>(StringComparer.Ordinal);
        var mshdField08Counts = new Dictionary<string, int>(StringComparer.Ordinal);
        var mprlUnk02Counts = new Dictionary<string, int>(StringComparer.Ordinal);
        var mprlUnk06Counts = new Dictionary<string, int>(StringComparer.Ordinal);
        var mprlUnk14Counts = new Dictionary<string, int>(StringComparer.Ordinal);
        var mprlUnk16Counts = new Dictionary<string, int>(StringComparer.Ordinal);
        var mprrValue2Counts = new Dictionary<string, int>(StringComparer.Ordinal);
        var decodedTileCounts = new Dictionary<string, int>(StringComparer.Ordinal);
        var otherLinkIdCounts = new Dictionary<string, int>(StringComparer.Ordinal);
        ...
        foreach (Pm4ResearchFile file in files)
        {
            ...
            foreach (Pm4MsurEntry surface in file.KnownChunks.Msur)
            {
                ...
                if (surface.IndexCount == 0 || ((long)surface.MsviFirstIndex + surface.IndexCount) <= msvtCount * 0 + file.KnownChunks.Msvi.Count)
                    msurToMsviFits++;
                else
                    msurToMsviMisses++;
            }
        ...
        List<Pm4FieldDistribution> fieldDistributions =
        [
        ...
        List<Pm4UnknownFinding> unknowns =
        [
