...
{
    public static Pm4ResearchFile ReadFile(string path)
    {
        return Read(File.ReadAllBytes(path), path);
    }

    public static Pm4ResearchFile Read(byte[] bytes, string? sourcePath = null)
    {
        ...
        while (offset + 8 <= bytes.Length)
        {
            ...
            {
                diagnostics.Add($"Failed to parse chunk '{signature}' at 0x{headerOffset:X}: {ex.Message}");
            }
    ...
    public static Pm4ExplorationSnapshot CreateSnapshot(Pm4ResearchFile file)
    {
        return new Pm4ExplorationSnapshot(
            file.Version,
            file.Chunks.Count,
            file.KnownChunks.Mslk.Count,
            file.KnownChunks.Mspv.Count,
            file.KnownChunks.Mspi.Count,
            file.KnownChunks.Msvt.Count,
            file.KnownChunks.Msvi.Count,
            file.KnownChunks.Msur.Count,
            file.KnownChunks.Mscn.Count,
            file.KnownChunks.Mprl.Count,
            file.KnownChunks.Mprr.Count,
            ComputeBounds(file.KnownChunks.Msvt),
            ComputeBounds(file.KnownChunks.Mscn),
            ComputeBounds(file.KnownChunks.Mprl.Select(static entry => entry.Position)),
            file.Diagnostics);
    }
    ...
    private static void ValidateStride(ReadOnlySpan<byte> payload, int stride, string signature, List<string> diagnostics)
    {
        ...
        if (remainder != 0)
            diagnostics.Add($"{signature} payload length {payload.Length} leaves remainder {remainder} on stride {stride}.");
    }

    private static Vector3 ReadVector3(ReadOnlySpan<byte> bytes)
    {
        return new Vector3(
            ReadSingle(bytes[0..4]),
            ReadSingle(bytes[4..8]),
            ReadSingle(bytes[8..12]));
    }
    ...
    private static int ReadSignedInt24(ReadOnlySpan<byte> bytes)
    {
    ...
    private static Pm4Bounds3? ComputeBounds(IEnumerable<Vector3> points)
    {
