...
public class Chunk : WowChunkedFormat
{
    ...
    public Chunk(FileStream file, int offsetInFile)
    {
        ...
        Letters = Encoding.ASCII.GetString(header.Slice(0, 4));
    ...
    public Chunk(byte[] wholeFile, int offsetInFile)
    {
        ...
        Letters = Encoding.ASCII.GetString(wholeFile, offsetInFile, 4);
    ...
    /// <summary>
    /// [PORT] Returns serialized chunk bytes: 4-letter FourCC + 4-byte size + data [+ pad].
    /// </summary>
    public byte[] GetWholeChunk()
    {
        int pad = (Data.Length & 1) == 1 ? 1 : 0;
        byte[] buffer = new byte[ChunkLettersAndSize + Data.Length + pad];
        Encoding.ASCII.GetBytes(Letters, 0, 4, buffer, 0);
    ...
    /// <summary>
    /// [PORT] Convert a data-relative offset to a file-relative offset.
    /// </summary>
    public int GetOffset(int offsetInData)
    {
        if (offsetInData < 0 || offsetInData > Data.Length) throw new ArgumentOutOfRangeException(nameof(offsetInData));
        return ChunkLettersAndSize + offsetInData;
    }
