...
AlphaWdtAnalyzer
Usage:
  Single map: AlphaWdtAnalyzer --input <path/to/map.wdt> --listfile <community_listfile.csv> [--lk-listfile <3x.txt>] --out <output_dir> [--cluster-threshold N] [--cluster-gap N] [--remap <remap.json>] [--web] [--export-adt --export-dir <dir> [--fallback-tileset <blp>] [--fallback-wmo <wmo>] [--fallback-m2 <m2>] [--fallback-blp <blp>] [--no-mh2o] [--asset-fuzzy on|off]]
  Batch maps:  AlphaWdtAnalyzer --input-dir <root_of_wdts> --listfile <community_listfile.csv> [--lk-listfile <3x.txt>] --out <output_dir> [--cluster-threshold N] [--cluster-gap N] [--remap <remap.json>] [--web] [--export-adt --export-dir <dir> [--fallback-tileset <blp>] [--fallback-wmo <wmo>] [--fallback-m2 <m2>] [--fallback-blp <blp>] [--no-mh2o] [--asset-fuzzy on|off]]
```
...
- `--remap <remap.json>`: AreaID remap file (see `DBCTool/docs/api.md`) to force explicit Alpha→LK mappings (e.g., `DBCTool/defs/053_to_335.remap.json`, `055_to_335.remap.json`).
...
> dotnet run --project .\AlphaWDTAnalysisTool\AlphaWdtAnalyzer.Cli \
  -- --input "..\..\test_data\0.5.5\tree\World\Maps\Azeroth\Azeroth.wdt" \
     ...
     --asset-fuzzy on \
     --remap ".\DBCTool\defs\055_to_335.remap.json"
```
...
- Provide `--remap <remap.json>` to apply explicit Alpha→LK mappings generated by `DBCTool` (see `DBCTool/docs/api.md`). Explicit mappings take precedence; entries listed under `ignore_area_numbers` are left unpatched.
...
- __AreaID CSV empty__: Provide `--remap` to apply explicit Alpha→LK mappings.
